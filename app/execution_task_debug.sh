...
if [[ "$WORKER_TOKEN" == "null" || -z "$WORKER_TOKEN" ]]; then echo "Worker login failed"; exit 1; fi

# 2. Resolve worker client_id
WORKER_ID=$(curl -s -H "Authorization: Bearer $ADMIN_TOKEN" "$BASE_URL/api/v1/users?limit=200" | jq -r ".data.items[] | select(.email == \"$WORKER_EMAIL\") | .client_id")
if [ -z "$WORKER_ID" ]; then echo "Worker ID not found"; exit 1; fi

# 3. Pick first task that has steps
