...
| 2 | `PLAN_declared_worker_states_phase2_derivation_20260729.md` | State machine + live reconcile + clock-out reconstruction read the new table (still inert — table is empty) | `needs_changes` — Opus review found F1/F2 (blocking: rebuild contradicts D4; `manually_recorded` leak), F3 (major: asymmetric idempotency). Findings in the plan's Review log; fix cycle via `codex_prompts/PROMPT_phase2_fixes.md`. Premature archive unwound. |
