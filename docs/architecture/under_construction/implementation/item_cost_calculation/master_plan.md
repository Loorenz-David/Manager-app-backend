...
| 6 | Legacy money migration & API bridge | `plans/phase_6_legacy_migration_api_bridge.md` | ⚑ (rows 31,32) | PROMPT_READY | 2026-08-11 | planner | journaled migrate-and-drop + reject-iff-non-null bridge. Projection r0 prompt authored 2026-08-14 (`prompts/reviewer/2026-08-14_phase6_projection_r0.md`; plan predates rounds 11-13 — axes: INV-V1/chain interaction, bulk created_at tie-breaker, live legacy-money measurement incl. the §10.2 refusal class, API-bridge greps re-run, enum ownership at the migration site, journal reversibility by state queries). **Projection r0 (Claude Opus 5): AMENDMENTS_REQUIRED** — 23 rows (5 blocking: D1 carrier mangled by parse helpers — executed table; D3 pc2 contradicts idempotency; D4 the census is NINE endpoints across TWO serializers incl. customer-detail linked_items; D5 inert mutation under the natural fixture; D6 four router bodies unlisted — silent-drop hazard), 2 owner cards RAISED AND ANSWERED in-session (attribution -> P3 pre-flight on the item's created_by_id; deleted prices stay deleted). Live census: 479 items, ALL legacy money at 0 non-NULL across 61 workspaces — every migration criterion vacuous on the live DB (seeded disposables + non-vacuity rows mandated); ZERO test dependents for the three keys. D15: the plan's frontend evidence was a misread (the body builder OMITS the amounts). Coordinator: intention round 14 folded (R14-1..R14-4); §6.4 carrier corrected + P3; squash seed gains Finding 6 (journal drop at squash); phase-9 gains the D22 frontend-mirror note; plan gains the GOVERNING amendments block; implementer r1 prompt authored (`prompts/implementer/2026-08-14_phase6_implement_r1.md`) |
