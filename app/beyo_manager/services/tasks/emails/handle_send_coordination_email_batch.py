...
async def handle_send_coordination_email_batch(raw: dict, task_client_id: str) -> None:
    payload = SendCoordinationEmailBatchPayload(**raw)
    logger.info(
        "coordination_email_batch_start | task_id=%s connection_id=%s thread_count=%d",
        task_client_id,
        payload.connection_client_id,
        len(payload.thread_ids),
    )
    ...
    async for session in get_db_session():
        ...
        if connection is None:
            logger.warning(
                "coordination_email_batch | missing_connection | task_id=%s connection_id=%s",
                task_client_id,
                payload.connection_client_id,
            )
            return
        ...
        message_result = await session.execute(
            ...
            .where(
                ...
                EmailMessage.thread_id.in_(payload.thread_ids),
        ...
        if not messages:
            logger.info(
                "coordination_email_batch | all_messages_already_attempted | task_id=%s",
                task_client_id,
            )
            return
    ...
    async for session in get_db_session():
        async with session.begin():
            ...
            sent_count = sum(1 for result in batch_result.results if result.success)
            failed_count = sum(1 for result in batch_result.results if not result.success)
            await write_audit(
                session=session,
        ...
        logger.info(
            "coordination_email_batch_done | task_id=%s attempted=%d sent=%d failed=%d",
            task_client_id,
            len(message_ids),
            sent_count,
            failed_count,
