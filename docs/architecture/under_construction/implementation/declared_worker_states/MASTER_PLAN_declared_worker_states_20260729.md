...
| 4 | `PLAN_declared_worker_states_phase4_clock_surface_20260729.md` | Explicit `/clock-in` + `/clock-out` routes, `GET /current` state endpoint, reasons filter, handoff validation | `needs_changes` — implemented at `20b11c7`, Opus review conformant on every functional item; R1 resolved by operator (stale acceptance criterion — handoff liveness is operator-owned, plans 4/6/7 amended), R4/R5/R6 low hardening + test gap. Fix cycle via `codex_prompts/PROMPT_phase4_fixes.md`. |
| 5 | `PLAN_declared_worker_states_phase5_device_auth_20260729.md` | `floor` app scope + non-expiring device token + permanent revocation semantics | `needs_changes` — Opus review at `549f480`: **N1 CRITICAL (revocation bypass: revoked floor token replayed as refresh cookie mints fresh tokens forever — executed exploit)**, N2 HIGH (socket auth skips blocklist), N3/N4 medium (jti unlogged; static-claims demotion — operator: accept+document), N5–N7 low/info. Fix cycle via `codex_prompts/PROMPT_phase5_fixes.md`. |
