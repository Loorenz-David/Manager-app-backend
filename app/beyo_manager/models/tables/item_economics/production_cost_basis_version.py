...
class ProductionCostBasisVersion(IdentityMixin, Base):
    ...
    monthly_paid_hours: Mapped[float] = mapped_column(Numeric(8, 2), nullable=False)
    planning_utilization_percent: Mapped[float] = mapped_column(Numeric(5, 2), nullable=False)
    cost_per_worker_minute_minor: Mapped[float] = mapped_column(Numeric(12, 4), nullable=False)
