...
```
plan: 2
state: PROMPT_READY (projection r0 routed — AMENDMENTS_REQUIRED; all 12 blocking, 10
       amendments and 11 notes folded; owner cards 1–3 answered → D13/D14/D15)
...
| # | Criterion | Kind |
|---|---|---|
| C1 | `order_list` tie: two sections sharing `order_list`, different names — exact order, **then again with insertion order reversed**. The measured production case. | DB |
...
| C6 | Governing step (D12): one `completed` + one later `pending` → row reads `pending`, `state_entered_at` from that step. Delete the rule → red. | DB |
...
| C9 | Mixed section: one `skipped` + one live → section stays **weighted**, skipped seconds in `worked_seconds`, **and `C` still charges them at task level** (B4/D15). | DB |
...
| C25 | **M3.9 rename** (P7, new): rename the section after the step exists → `section_name` is the new name, `section_name_snapshot` the old, on the same row. The two are identical on all 2833 live rows, so only a rename can distinguish them (letter-verification rule). | DB |
