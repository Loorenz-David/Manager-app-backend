...
from beyo_manager.domain.shopify.enums import ShopifyIntegrationStatusEnum, ShopifyWebhookSubscriptionStatusEnum
...
from beyo_manager.domain.shopify.scopes import compare_requested_and_granted_scopes, has_all_required_scopes
...
def serialize_shopify_webhook_subscription(row: "ShopifyWebhookSubscription") -> ShopifyWebhookSubscriptionResult:
...
def serialize_shopify_scope_status(row: "ShopifyShopIntegration") -> ShopifyScopeStatusResult:
    comparison = compare_requested_and_granted_scopes(row.requested_scopes or (), row.granted_scopes or ())
    return ShopifyScopeStatusResult(
        ...
        has_all_required_scopes=has_all_required_scopes(row.requested_scopes or (), row.granted_scopes or ()),
        shop_status="outdated" if row.status in {ShopifyIntegrationStatusEnum.SCOPES_OUTDATED, ShopifyIntegrationStatusEnum.NEEDS_REAUTH} else "up_to_date",
    )
...
def _derive_webhooks_status(
    ...
    subscriptions_by_topic = {subscription.topic: subscription for subscription in webhook_subscriptions}
    if any(subscription.status == ShopifyWebhookSubscriptionStatusEnum.FAILED for subscription in subscriptions_by_topic.values()):
        return "has_failures"

    for definition in SHOPIFY_WEBHOOK_REGISTRY:
        if not definition.enabled:
            continue
        if not has_all_required_scopes(definition.required_scopes, integration.granted_scopes or ()):  # type: ignore[arg-type]
            continue
        subscription = subscriptions_by_topic.get(definition.topic)
        if subscription is None or subscription.status != ShopifyWebhookSubscriptionStatusEnum.ACTIVE:
            return "needs_sync"
    return "synced"
...
def serialize_shopify_shop_integration(
    ...
    scopes_status = "outdated" if row.status in {ShopifyIntegrationStatusEnum.SCOPES_OUTDATED, ShopifyIntegrationStatusEnum.NEEDS_REAUTH} else "up_to_date"
