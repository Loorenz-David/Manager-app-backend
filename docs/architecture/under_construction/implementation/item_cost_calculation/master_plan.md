...
| 8 | Status & results | `plans/phase_8_status_results.md` | ⚑ (rows 9,26–30,34) | NOT_STARTED | 2026-08-12 | coordinator | status query, result handler, §8B boundary emissions (round-6 fold: READY/reopen hooks, widened guard, C6b). Projection r0 prompt authored 2026-08-14 (`prompts/reviewer/2026-08-14_phase8_projection_r0.md`; plan predates rounds 13–16 and phases 4B–7 — axes: D23 twelve-value C7, §8A.3/§8A.4 idempotency+replay, §8B boundaries vs the CURRENT transition core, P-H worker-payload money boundary, phase-7 N6 two-loader pin, R2-N2 seam hardening, criteria under §9 incl. the ten phase-7 rules, decidability). **Projection r0 (Claude Opus 5): AMENDMENTS_REQUIRED** — 7 blocking / 12 should-fix / 6 notes, 2 owner cards; env re-verified (head unchanged, NO migration needed, `uq_item_cost_results_task_id` is a UNIQUE constraint, collection 2099/2100 reconciles §10 exactly, payload-key greps zero hits, `on_conflict_do_update` zero repo precedent, graph 166/239 all human_confirmed). Standouts: L1 C7's eleven-value list vs the twelve-member enum PLUS the two-producer composition hazard (`selection.status is OK` must never leak into the payload); L2/L3 task 5 (lifetime read) had ZERO criteria and five undecidable axes; L4 the reopen hook's target is sync/session-less (signature change + P-E amendment); L5 the terminal emit as worded lands inside `if target_user_ids:` (a task with no notification recipients would silently never get a result row); L7 the all-roles budget-status route breaks the `_ROUTES` shape (cheapest green would silently reverse card 4). Coordinator routed all 25 rows 2026-08-14: owner cards → **R17-1** (result shows whenever a row exists, boundary-labelled — READY counts as resolved from the item-cost perspective) + **R17-2** (DELETE status re-resolved, never hand-written — one rule, no drift; never-priced == deleted-price, asserted literally); intention round 17; §9 P-E amended (the two named call-site adaptations admitted); plan GOVERNING block A1–A17 (incl. delegations decided: independent worker service, `include_monetary_step_fields` reuse, loader equality-property pin with stated exclusion + non-vacuity, named-constraint upsert, worker result-block 5-key set, N4 one-file extension); 4B N3 → phase 9. Implementer prompt authored (`prompts/implementer/2026-08-14_phase8_implement_r1.md`) |
