...
async def _update_item_upholstery_in_session(
    ctx: ServiceContext,
    request: UpdateItemUpholsteryRequest,
) -> ItemUpholstery:
    """Update an ItemUpholstery inside the caller's transaction."""
...
async def update_item_upholstery(ctx: ServiceContext) -> dict:
    ...
    async with maybe_begin(ctx.session):
        iup = await _update_item_upholstery_in_session(ctx, request)
