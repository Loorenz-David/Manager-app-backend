...
2. `docs/architecture/under_construction/implementation/task_budget_overrun_signal/plans/plan_1.md`
   exists and its Review log contains only the coordinator's two 2026-08-24 projection-fold
   entries.
...
and no-work-ahead boundary; both exact `int`/`Decimal` money-call seams; four-state precedence
with both populated pairs; the wire-only currency sentinel; and the full public API surface.
Configuration/plumbing receives only the depth needed to establish it has no hidden decision.
...
`role: projection`, `round: 2`, `date`, `verdict` (`PROJECTED_CLEAN` or
