...
| **The test suite runs against the development database** | Nothing sets `APP_ENV=testing`, so `pytest` loads `.env` and writes into `beyo_manager` — 1,715 workspaces and 1,497 users of accumulated test data. `architecture/15_testing.md` forbids this in as many words. It also means the 15 failing integration tests cannot be told apart from database sediment. Full diagnosis, evidence and fix plan: [debugging/test_suite_runs_against_the_development_database_20260801.md](debugging/test_suite_runs_against_the_development_database_20260801.md). |
| **`make reset-db` is a no-op** | `run_reset_db()` guards against running without `confirm`, then only calls `log_event`. It resets nothing, despite the name. Implement it or drop the target. |
...
| **`domain/emails/arrival_notifications.py` does I/O from the domain layer** | It opens DB queries, enqueues tasks and pushes sockets, which `architecture/08_domain.md` forbids outright — the domain layer is defined as pure functions with no I/O. It is a service living in `domain/`. Found while migrating its socket call off `worker_emitter`; the transport is now conformant, the layer is not. Moving it wants a home for the impure half (`services/infra/events/` alongside the other data-carrying emitters) and touches two callers in `services/tasks/`. |
...
| **Worker-shift events bypass the event bus** | `worker_shift_realtime.py` calls `realtime_push` directly, so shift events reach sockets but never `audit_handler` or `webhook_handler` — adding one to the audited allowlist would do nothing. Deliberate: the payload is a full state object re-read from the database so the floor app need not refetch, which does not fit the bus's `{client_id, **extra}` shape or its "keep `extra` minimal" guidance. Sanctioned and bounded in `architecture/11_infra_events.md` § Data-carrying emitters. Two ways out if it ever needs auditing — make the events minimal notifies and let clients refetch `/current`, or put the state in `extra` and accept a redundant `client_id` key. Revisit when a second candidate for this pattern appears, or when a shift event needs an audit trail; not before. |
