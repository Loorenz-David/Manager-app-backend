...
| 5 | Valuation surface | `plans/phase_5_valuation_surface.md` | ⚑ (rows 15,16 — valuation chain; 34) | **APPROVED** | 2026-08-14 | planner; Codex; reviewer r1 (Claude); Codex (fix r1); re-reviewer r2 (Claude); Codex (fix r2); re-reviewer r3 (Claude) | ItemValuation chain command + preview implemented. Projection r0 prompt authored 2026-08-13 (`prompts/reviewer/2026-08-13_phase5_projection_r0.md`; folds §7C shipped signatures, N-d live re-measure, forward-notes block, head 5caae620088c, baseline 1927/23/1). **Projection r0 (Claude Opus 5): AMENDMENTS_REQUIRED** — 16 rows (6 blocking), 7 notes, 2 owner cards; probes EXECUTED (persisted-rate Q2-tie fixture computed 76800.20 vs 76800.00; self-FK teardown hazard disproven; N-d re-measured 53/225/193 of 471; L7 parser leading-token loss verified; L18 INV-V1 premise re-confirmed live). Coordinator: cards ANSWERED and folded as intention round 13 (R13-1 preview key + computable-state numerics + first-save-is-version-1 no-confirmation pin, confirmed against a coordinator story; R13-2 deleted rows hidden from history, INV-V1 current predicate, created_at DESC+client_id DESC); §9 P-B refined; §6.4 += item_valuation.created/.deleted; §6.5 += resolve_economics_selection/EconomicsSelection + ITEM_READINESS_PRECEDENCE/resolve_item_economics_status + the response envelope + L15 correction; plan gains the GOVERNING amendments block (L1-L16 + notes + delegations); implementer r1 delivered set/delete/history services, persisted-rate preview, request validation, three ADMIN/MANAGER routes, tests, router README, and additive architecture delta (5 nodes, 7 edges; revision b5e6fe094cae). Implementer r1 (Codex, `8b4ac06`, final; handoff after): full surface (set/delete commands, history query, 3 routes, selection + readiness resolvers per §6.5, preview envelope with the persisted-rate fixture 76800.20, R13-2 history pins, audit events); focused 111; graph delta 5 nodes + 7 edges (`b5e6fe09…`, pending). Coordinator: consumed — perimeter exact (16 files), 3 restored hashes byte-identical, but TWO findings routed to review: arithmetic off by one (claimed 1951+23 vs 1973 selected; expected 1950/23/1) and the ledger declares 3 mutations vs ~10 named in the amendments (L1/L2x2/L15/L16/L7/audit owed — the central review probe); review r1 prompt authored (`prompts/reviewer/2026-08-13_phase5_review_r1.md`). **Review r1 (Claude Opus 5): CHANGES_REQUESTED** — perimeter exact (16 files, tree clean, `git diff 8b4ac06..HEAD -- app/` empty, 3 declared hashes byte-identical), ruff clean, suite **1950/23/1** with the failure set byte-identical to the phase-1 baseline (**P5-A resolved: the handoff's 1951 is derived-not-read — +23 tests, not +24**), DB at head, graph read-only zero delta. **P5-B resolved: 7 owed mutations run, 4 do not bite.** 4 blocking — B1 `delete_item_valuation` omits `is_deleted = false` from INV-V1's predicate, so after delete-then-reset the live valuation is permanently undeletable (reproduced; one-clause fix verified 346/0); B2 C5's routed 12-value enumeration (L4) not built (3 statuses in one monolithic test, no P-V ids, no `item_cost_evaluations` assertion — reviewer probes for the missing rows pass, so the code is right and the evidence absent); B3 the L15 structural row does not exist (inlining a snapshot read leaves 345 green); B4 the three-way currency equality has no per-clause arbiter (all three clause drops green) and the ids misname their pairs — equality is transitive so the middle clause is provably redundant; verified 2-clause correction gives both clauses sole-cause arbiters. 5 should-fix — S1 the purchase-cost↔currency adjacent pair unarbitrated (swap green; verified fixture supplied); S2 C6 unbuilt (drop *and* reverse of the history ordering both green — the only assertion is a one-element list); S3 C4's re-set row absent (the row that would have caught B1); S4 C2 cannot count INV-V1's predicate and neither race path's observable is asserted; S5 C3 missing the missing-currency and three accept rows. 8 notes (N1 L12's *named* mutation is inert — only raw re-division bites, verified fixture `13.0000`→`76923.08`; N2 DELETE's hardcoded `item_unvalued` → phase 8; N4 pre-checkpoint dev-DB residue → closeout purge, not a teardown defect; N6/N7 graph read-boundary + stale `domain-item-economics` anchor). Verified correct: chain order, race identity on the real conflict path, L1 delegation (12-node blast radius), L2(i) structural independence, L7 exact leading token, L9 both audit events, P-R role gates (exactly 3 MANAGER rows), R13-2 filter, L12 arithmetic, L5/L6/L18/L20. Anchor-spans service delivered for all 12 pending items (9 exact, 3 corrected) plus the undeclared stale link. Lessons: routed amendments need ledger rows (P-I ext); transitive relations enumerate by state not clause; a named mutation must be checked against the implementation it meets (P-Q ext); a monolithic test cannot discharge an enumerated criterion (P-V ext). **Review r1 (Claude Opus 5): CHANGES_REQUESTED** — 4 blocking, 5 should-fix, 7 notes, 0 owner cards. B1 REAL defect: delete-then-reset makes the new price undeletable (delete predicate missing is_deleted=false; reachable via PUT-DELETE-PUT-DELETE; one-line fix executed+verified 346/0). B2 C5's 12-value enumeration unbuilt (3 statuses in one monolithic node); B3 the L15 structural row absent (inline snapshot read leaves 345 green); B4 the 3-clause currency check has NO per-clause arbiter (transitivity — provably unfixable pairwise; verified 2-clause correction). S1 precedence pair 2 unarbitrated; S2 C6 unbuilt (ordering droppable/reversible green); S3 the re-set row absent (B1's would-have-caught); S4 races can't count; S5 C3 missing 4 rows. N1 L12's NAMED mutation inert by construction (calculator re-derives the persisted value exactly; corrected fixture 13.0000 -> 76923.08). P5-A resolved: true suite 1950/23/1 — the handoff's 1951 was DERIVED (1927+24), not read. P5-B resolved: 7 owed mutations run, 4 don't bite. Graph: 12 claims all TRUE, anchor service delivered (2 imprecise/2 wide), N6 five missing reads_from edges, N7 domain source-link stale (L1 moved the function; re-link resolve_economics_selection 80-126). Coordinator: lessons folded (P-I 6th ext N-named=N-rows; P-AA transitive states not clauses; P-Q ext mutation-vs-implementation; P-V 2nd ext monolithic nodes); plan gains the GOVERNING fix-r1 amendments block; fix-r1 prompt authored (`prompts/implementer/2026-08-14_phase5_fix_r1.md`). Fix r1 (Codex, `a0cebde`, final): production diff exactly TWO lines (delete predicate +1, redundant currency clause -1); C5 rebuilt as 12 ids; L15 structural row; shared delete-then-reset fixture (C4/C6/L13); S1/S4/S5; N1 fixture 13.0000->76923.08; 14-row ledger, all previously-inert mutations now red. Coordinator: consumed — perimeter exact, arithmetic READ (1968/23/1 = 1991 selected exactly), and BOTH final production hashes byte-identical to files the r1 REVIEWER produced (ab9aebbe = the B1 correction file; 75087586 = the M5.b 2-clause probe) — the fix shipped the verified corrections to the byte; re-review r2 prompt authored (`prompts/reviewer/2026-08-14_phase5_rereview_r2.md`). **Re-review r2 (Claude Opus 5): CHANGES_REQUESTED — 0 blocking, 1 should-fix, 7 notes.** Perimeter exact (7 files; tree clean; `git diff a0cebde..HEAD -- app/` empty; all five declared final hashes match); production diff exactly two lines and both production files byte-identical to r1's own correction-verification files; ruff clean; suite re-run **1968/23/1** (collection 1991+1) with the failure set byte-identical to the phase-1 baseline; focused 363 (r1: 345); DB at head; graph read-only zero delta (`b5e6fe09…`, 153/195, 12 pending). **All four blocking and all five should-fix r1 findings CLOSED**, each re-proven by a mutation green in r1 and red now, every declared mutant hash reproducing byte-identically: B1-revert (`23cfe90f…` = the old unfixed file) reds the chain; C5's 12 P-V ids map one-for-one onto §11A.4-as-amended with 3 recorded judgments and sole-predicate verified on 5 sampled rows; M4a reds the new structural row; both currency drops (`ee228801…`, `796ad66e…`) and the precedence swap (`bf241b9d…`) red exactly their own rows; M8/M8b red; path (i) asserts `close rowcounts == [0,1]` and both blocks count INV-V1 `== 1` (race subset twice + whole file twice, ten tables flat); **M10 — L12's *named* mutation, inert in r1 — now reds** via the `13.0000`/`76923.08` fixture. **1 should-fix (S1):** the L15 structural guard builds `module_sources` over exactly L15's two roots (24 modules) and then never asserts the guard against it — keeping the resolver call while adding a second unmediated snapshot read in the same module (`e1ca0625…`) or a snapshot-classifying helper in `delete_item_valuation.py` (`88c9f5aa…`) each leave **363 green**; the named mutation bites but the property does not (P-J). Verified 6-line correction executed: baseline 9 passed, M4a/M4b/M4c each redden. 7 notes: N1 C5's row numbers are a hybrid of the pre-round-12 and post-§7C.3 numberings and map to no single authority (L4's own example is pre-round-12) → next touch + plan correction; N2 dropping `client_id DESC` alone leaves 363 green — unreachable today, load-bearing once **phase 6** bulk-creates valuations → phase-6 forward note; N3 the renamed currency ids name the pair held EQUAL while the clause each arbitrates is the other one (r1's own wording) — one comment closes it; N4 two M4 mutants under one label, the ledger's red set belonging to the one that did not exercise the row under review (P-I ext); N5 race path (ii) rowcount observable absent (gate-guaranteed); N6 phase-2 citation is prose not a node id; N7 r1 carry-forwards unchanged. Anchor spans: two moved (+1 in `delete_item_valuation.py`) — node **17–44**, `--writes_to-->` **39–42**; all others re-verified unchanged; stale `domain-item-economics` link unmoved (re-link `resolve_economics_selection` **80–126**). Lessons: a structural guard is graded on the reader added *later*, so a module-set property must be quantified over the set (P-J ext); parametrize row numbers must cite a numbering scheme that still exists (P-V); a reused mutation name needs a mutant hash beside each red set (P-I ext); process — a reviewer's background suite run must not overlap its own probes. **Re-review r2 (Claude Opus 5): CHANGES_REQUESTED — 0 blocking, 1 should-fix, 7 notes.** All nine r1 findings CLOSED, each re-proven by the formerly-green mutation now biting (B1 through the shipped state; C5's 12 ids map 1:1 with judgments; both currency clauses sole-cause; M8/M8b/M10 all red; race counts INV-V1 == 1 with the [0,1] blocking observable; suite 1968/23/1 re-run, baseline byte-identical; self-caught background-run measurement error corrected). S1: the L15 guard builds the module set then never quantifies over it — a SECOND unmediated reader survives (M4b/M4c 363 green); six-line correction executed (9 passed; all three shapes red). Notes: N1 hybrid id numbering (L4's own example was pre-round-12 — plan corrected); N2 client_id-DESC tie-breaker unarbitrable until phase 6 bulk-creates (forward note placed); N3 equal-pair ids invert under casual read (comment ride-along); N4 two mutants under one label (P-I 7th ext). Anchor spans: only delete_item_valuation shifted (+1 -> node 17-44, writes_to 39-42); all else final. Coordinator: lessons folded (P-J 2nd ext quantify-over-set; P-V numbering rule; P-I 7th ext; reviewer foreground-runs practice); fix-r2 prompt authored (`prompts/implementer/2026-08-14_phase5_fix_r2.md`) — one test-side item | Fix r2: quantified L15 structural guard + N3 clause comment; 1968/23/1 full suite; graph zero delta. Fix r2 (Codex, `e71b5b4`): ONE test file (+11 lines, final da1c4e28) — quantified unmediated=={} guard + N3 clause comment; all three M4 shapes red (governing hashes transcribed, own equivalent mutants declared separately with hashes); 9/363/1968-23-1. Coordinator: consumed, perimeter exact; row-pipe defect repaired (the fix session's tracker append dropped the trailing cell delimiter); re-review r3 prompt authored (`prompts/reviewer/2026-08-14_phase5_rereview_r3.md`) — trivial delta, APPROVED expected. **Re-review r3 (Claude Opus 5): APPROVED — 0 blocking, 0 should-fix, 1 note.** Perimeter exact (3 files; one code file `test_configuration.py` +11, final `da1c4e28…` matching the declaration; tree clean; `git diff e71b5b4..HEAD -- app/` empty; **zero production change** — the three production files byte-identical to r2's approved state); ruff clean; suite re-run foreground on a hash-verified-clean tree **1968/23/1** (collection 1991+1) with the failure set byte-identical to the phase-1 baseline; focused 363; DB at head; graph read-only zero delta (`b5e6fe09…`, 153/195, 12 pending). **S1 CLOSED:** the guard quantifies over all of `module_sources`, verified to resolve to 24 modules across both L15 roots with exactly one mediated reader today (satisfiable, not trivially true); all three shapes redden **exactly** the guard with zero collateral — M4a `df1f79b3…` (red in r2), **M4b `1309a947…`** and **M4c `e02b028e…`** (both 363-green in r2); byte provenance stated honestly (M4a reproduces r2's hash; M4b/M4c same mutations, different bytes). **Vacuity checked unbidden:** breaking the root resolution (`fd389abf…`) dies on `FileNotFoundError` rather than passing silently, so the unconditional `set_path` read + membership assertion are load-bearing non-vacuity arbiters. N3 comment verified accurate against r2's executed clause→id mapping. **1 note (N1):** the `services` filter tests `path.parent.name`, leaving `services/commands/item_economics/requests/__init__.py` outside the walk — executed (M4d `a5261ce8…`): a snapshot read there leaves 363 green. Note, not a finding: that module holds pydantic request models that never hold an `Item` ORM row. One-predicate correction verified (`"item_economics" not in path.parts`) — reddens M4d, clean tree stays at 9. -> next touch. Anchor spans FINAL and unchanged from r2 (test-only cycle): set node 102-168; delete node 17-44; endpoints 229-241 / 244-256 / 258-269; set writes_to 128-159; delete writes_to 39-42; history reads_from 23-31 and returns 32-33; three router-anchored edges. Stale `domain-item-economics` link unmoved -> re-link `resolve_economics_selection` **80-126**. Lessons: a criterion mandating a property over a *discovered* set owes a row proving the set is non-empty (P-J ext); a scope predicate should use the wording's semantics (`services/**/item_economics/` vs "immediate parent"). **Phase 5 closed:** B1 (a real, reproduced data-access defect), B2, B3, B4 and S1-S5 all resolved and independently re-verified across four rounds; every governing round-0 amendment L1-L16 now has a live arbiter; zero findings outstanding. Carry-forwards routed in the r3 handoff (r3 N1; r2 N1/N2 -> phase 6 /N4/N6; r1 N2 -> phase 8, N4 -> closeout purge, N5 -> phase 9, N6/N7 -> graph pass) |
| 6 | Legacy money migration & API bridge | `plans/phase_6_legacy_migration_api_bridge.md` | ⚑ (rows 31,32) | **APPROVED** | 2026-08-14 | Codex; reviewer r1 (Claude Opus 5); Codex (fix r1); re-reviewer r2 (Claude Opus 5) | B1/B2 and S1–S4 resolved: no-valuation eligibility, independent unmigrated guard, authority-parametrized migration cases, ORM-backed nine-surface serializer census, three TestClient item-router rows, synthetic tie arbiter, and N2/N4/N5/N6/N7/N8 corrections. Focused 58 passed; full non-e2e 2012 passed / 23 established baseline failures / 1 deselected; ruff and diff checks clean; configured DB at `be9dfe42a035` head with journal 0, legacy columns 0, and disposable phase-6/reviewer databases 0. Fix handoff and final-hash mutation ledger recorded in the phase plan and handoff.. Fix r1 (Codex, `51d8b7c`, final `a3228a85…`): B1 guard on independent constructions (skip mutation now ABORTS + rolls back); B2 predicate per R15-1 with all four valuation-state rows (soft-deleted-only now SKIPPED); S1 census rows real (R3 probe reds exactly upholstery-orders); S2 report contents (R2 mutant reds all three); S3 three TestClient rows; S4 parametrized; N2/N4-N8 taken; ledger vs FINAL hashes; focused 58; full 2012/23/1 (= 2035 selected, coordinator re-collected exact, +15). Coordinator: consumed — drop-file +7 was N8 (the fix prompt's perimeter line contradicted its own items list; implementer resolved sensibly — coordinator inconsistency recorded); re-review r2 prompt authored (`prompts/reviewer/2026-08-14_phase6_rereview_r2.md`) **Re-review r2 (Claude Opus 5): APPROVED — 0 blocking, 0 should-fix, 4 notes, 0 owner cards.** Perimeter exact (8 files, all declared; `git diff 51d8b7c..HEAD -- app/` empty; tree clean before and after every probe; both production files match the declared final hashes `a3228a85…` / `65f93a21…`); suite re-run foreground **2012/23/1** with the failure set byte-identical to the phase-1 baseline; collection +15 exact (43 nodes in the three phase-6 files, up from 29, plus N7's extracted tie test); ruff clean on the fix perimeter; DB at head `be9dfe42a035` (journal 0, legacy columns 0, zero disposables); graph read-only, revision `4eb1d8d0…` byte-identical to r1, 7 pending. **B1 CLOSED** — the guard is now built from the items/valuations side and compared against zero; the identical `rows[1:]` mutation that exited 0 in r1 (`a3228a85…` → `6c8ce6e7…`) now exits 1 with `left 1 eligible item(s) unmigrated` and rolls back to `5caae620088c`; 13 rows green at baseline (not over-tight). **B2 CLOSED** — `_NO_VALUATION` matches R15-1 verbatim; re-derived independently on a seeded disposable with the UNMUTATED migration: never-valued → migrated (non-vacuity arbiter), current-valued → journaled only and the existing valuation still reads 777 (identity), soft-deleted-only → **SKIPPED** (was re-valued in r1), superseded-only → SKIPPED. **S1 CLOSED** — the R3 mutant (`99a34732…`, byte-identical to r1's) reddens EXACTLY the `upholstery-orders` row, eight green (all 27 were green in r1); rows consume endpoint source coordinates; fixtures are ORM `Item`. **S2 CLOSED** — emptying the three id lists (`ebb503d8…`) reddens all three refusal rows; each row asserts its own id plus `client_ids=[]` for the other two classes. **S3 CLOSED** — three TestClient rows, exact envelope. **S4 CLOSED** — 12 authority-named nodes replace two for-loops. r1 N5/N6/N7 CLOSED; N1 (ledger accuracy) CLOSED — all three fix rows declared against FINAL hashes. **Adversarial depth, unbidden:** deleting the journal back-link UPDATE (`69b93980…`) creates a valuation the guard alone cannot see, but the N5 identity set bites (1 failed / 12 passed) — labour correctly divided. **4 notes:** N1 r1's N8 is HALF closed and the fix record claims otherwise (`Revises: 5caae620088c` vs `down_revision = "5420acc6a7b3"` — logger done, docstring not; this entry is the correction); N2 the head enum-user assertion has an AND/OR precedence bug (verified: a bogus typname still returns 1, should be 0 — the asserted total is still right); N3 `j.valuation_client_id IS NULL` is now redundant (removing it leaves 13 green), so r1 N4's single-cause half stays open though the post-conditions DO re-run on pass 2; N4 CWD-relative fixture path + line-pinned source windows in the serializer test. Journal-node anchor spans MOVED with the +23: **44-74** and **203-246** (were 45-75 / 200-243); all other spans anchor in untouched files and re-verified exact. Lessons: a fix cycle's own record is evidence and gets verified like any other claim (P-I ext from hashes to prose); an `OR`ed structural assertion needs its precedence arbitrated by falsifying one conjunct; when a fix replaces a predicate, ask whether each surviving conjunct still has a case that depends on it. **Phase 6 closed:** both blockers were real silent-money-loss defects, both are now proven fixed by the mutations that were green in r1 and bite now; carry-forwards routed in the r2 handoff. **Re-review r2: APPROVED — 0/0/4 notes.** B1 CLOSED (skip mutation: exit 1, rolled back, journal absent — vs r1's exit 0); B2 CLOSED (all four valuation states re-derived on disposables with the UNMUTATED migration; soft-deleted-only SKIPPED; R15-1 verbatim at intention:1575-1588); S1-S4 closed with shipped arbiters (R3 reds exactly upholstery-orders; R2 reds all three refusal rows with per-class content); unbidden depth: journal back-link deletion caught by the round-trip identity row; R2-P7 found one redundant conjunct (N3, no defect). Notes: N1 r1-N8 half-closed while the record claimed full (record corrected in the Review log; code line -> phase 9); N2 OR-precedence in the enum assertion; N4 CWD-relative fixture path. Suite 2012/23/1 foreground; +15 exact; graph zero delta, journal spans corrected 44-74/203-246. Coordinator: CLOSED OUT — 10 artifacts to archive/plan_6/; §10 current-head entry (be9dfe42a035) + suite baseline updated; N3 -> only-if-cheap; N9 + docstring -> phase-9 notes; §9 += P-I 8th ext (fix records are evidence), precedence-arbitration + surviving-conjunct rules; no purge owed (journal 0 rows) |
| 7 | Evaluations | `plans/phase_7_evaluations.md` | ⚑ (rows 2,5,7,10,14,16,17,19,21–25) | **APPROVED** | 2026-08-14 | planner; projectionist r0 (Claude); coordinator; Codex implementer r1; reviewer r1 (Claude Opus 5); Codex (fix r1); re-reviewer r2 (Claude Opus 5) | commit tx, projections, promotion, auto path, mirror. Projection r0 prompt authored 2026-08-14 (`prompts/reviewer/2026-08-14_phase7_projection_r0.md`). **Projection r0: AMENDMENTS_REQUIRED** (23 rows: 12 blocking, 10 should-fix, 1 note; 1 owner card; env re-verified incl. head `be9dfe42a035`, all economics tables 0 rows, PG 18.4; payload-key greps zero hits; N15 wording CLEAN; C3 admission total as written). Fully routed 2026-08-14: **R16-1** (owner card 1: commit visible in the team task flow; coordinator fold = TASK-linked HistoryRecord via `task_flow_records`'s always-on TASK condition — zero migration, D8's enum branch dissolved); **R16-2** (D4 mirror-race hole at READ COMMITTED closed: §7B.1 step 4 takes the valuation FOR UPDATE); **R16-3** (D7 pre-check restated as the resolver, total by construction; D17 both auto-path log lines verbatim); **R16-4** (D9 auto path appends to `pending_events` after the savepoint exits; file fence amended). Registry: §6.4 += status→identity mapping table + `ITEM_COST_ITEM_MISSING_MAJOR_CATEGORY` (4B N-c resolved) + `item_cost_evaluation.*` audit rows + `_common.py` evaluations-index row with the uniform conflict sentence standing; §6.5 += `_commit_item_cost_evaluation_in_session` helper (D10), `_common.py` extractions with P-Z property tests (D19), rate-snapshot RECOMPUTE decision (D13), serializers/requests/`label`/projection-`source` (D18), evaluations-read pins ×4 (D15), rederive-marker consumer = phase-7 read (D16). Plan GOVERNING block: C2 P-S judgment + task-lock criterion (D3), C5 sixth row (D4), C6/C9 re-enumerated vs §7C.2 + §11A.4 tables (D11/D7), C9 overflow fixture replacing the inert patched-calculator branch (D6), FOR SHARE two-chain outcome criterion (D5), router `_ROUTES` completeness arbiter (D14), C10 dispatch seam + after-commit observable + TASK-linked record (D20), C8 promotion admission/lock/ownership + named byte-unchanged column set (D21/D22). D23 → phase-8 forward note (C7 says eleven, vocabulary is 12). **Graph-count correction (coordinator record):** closeout figures were one high on each — live counts 155 nodes / 200 edges (projectionist measured, coordinator re-verified via `archgraph_status`), revision `53261a23…`, 2 pending. Implementer r1 (Codex): four commands, projections/promotion, task auto-commit savepoint, evaluation history/marker, five routes/README, shared valuation extraction, focused 94 tests; full non-E2E 2034 passed / 23 established failures / 1 deselected **[S4 CORRECTION 2026-08-14: both figures WRONG — reviewer's foreground runs read focused 92 (router 88 + integration 4), full 2037/23/1, delta +25 reconciled exactly; see the review-r1 entry below]**; DB at head `be9dfe42a035`; one batched graph delta recorded (11 nodes, 39 edges). Coordinator consumption (2026-08-14): checkpoint `a7f421f` (17 files = declared perimeter exactly; deposits handoff-only; `git diff a7f421f..HEAD -- app/` empty); BOTH declared mutation hash pairs re-verified byte-identical; graph state matches declaration (166/239, 52 pending, rev `0a71061…`, 2 pre-existing untouched). Consumption findings → review probes: (1) suite/focused numbers disagree THREE ways across handoff (97 / 2037), Review log (88+4=92), and this row (94 / 2034) — P-L transcription class, reviewer re-runs; (2) concurrency mutations honestly deferred to review (P4-3 precedent) but C1 row 1b's single-session snapshot-source mutation is missing from BOTH the run list and the deferral list; (3) implementer added unregistered name `auto_commit_item_cost_evaluation_in_session` — registered post-hoc in §6.5, reviewer verifies it holds the pre-check and nothing else; (4) plan header state slip (NOT_STARTED at IMPLEMENTED) corrected by coordinator. Review r1 prompt authored (`prompts/reviewer/2026-08-14_phase7_review_r1.md`; probes P7-1…P7-10 incl. the reviewer-owned concurrency mutation pass: task lock :111, valuation lock :183, read=True :149/:155 row-1-only, direct-INSERT conflict translation). **Review r1 (Claude Opus 5): CHANGES_REQUESTED** — 3 blocking, 5 should-fix, 8 notes (`handoffs/reviewer/2026-08-14_phase7_review_r1_handoff.md`). B1: the §7B.4 mirror write is not gated on `kind`, so creating a PROJECTION with a price override permanently supersedes the item's real valuation (irreversible; graph delta corroborates — it records `writes_to → item_valuations` for commit/promote, not for projection creation). B2: ~52 of ~60 amended C1–C14 rows have no arbiter (4 integration tests + 21 router nodes for the whole phase); reviewer built and ran the missing rows — all pass except B1's, so it is a proof gap, not a correctness gap. B3: C8's byte-unchanged check uses the same session, which A3/C8 forbids, and the shipped fixture never commits. S1/S2/S3: C2's prescribed fixture and C11's and C5-row-6's named mutations are all INERT as written — FK `KEY SHARE` and the commit's own mirror UPDATE mask the deleted locks (P-T lesson, three plan corrections owed BEFORE the fix prompt). S5: promotion's cross-task guard keys on an input the projection-keyed route never sends. **S4 — this row's numbers were wrong on both figures**; reviewer's own foreground runs: focused **92**, create_task integration **29**, phase-5 valuation **54**, full non-E2E **2037 passed / 23 failed / 1 deselected**, failure set byte-identical to the phase-1 list (23/23), delta **+25** reconciled (21 router nodes + 4 integration). Every named mutation executed with hash pairs and observed node ids (7 rows incl. all four deferred concurrency mutations); C1 row 1b's row EXISTS and its mutation bites — it was only missing from the ledger lists. All production files reverted (`git diff -- app/beyo_manager/` empty); economics tables 0 rows before and after; DB at head; graph READ-ONLY, zero delta, nothing adjudicated. Coordinator (post-review, 2026-08-14): B1 independently CONFIRMED by inspection (the mirror block sits ungated between kind-gated blocks — P-AB's exact shape); reviewer perimeter exact (3 docs; app/ byte-identical to `a7f421f`); probe files preserved out of the volatile session scratchpad into `probes/reviewer_r1/` (sha256s recorded — the fix cycle adopts them); lessons L1–L6 folded into §9 (P-T 3rd ext lock MODE, P-Q 4th ext fixture-vs-engine, P-R 2nd ext committing harness, **P-AB** parameter-gated helper effects, deferral rule, P-L 2nd ext tracker-figures-at-consumption — the last one is the coordinator's own miss); plan gains the GOVERNING fix-r1 block F1–F9 (S1/S2/S3 criterion corrections applied BEFORE the fix prompt per L5; S5 decided: dead branch deleted + cross-WORKSPACE row; N3 decided: literal `no_primary_item`, no enum member; N4 decided: which-module claim deleted, fired-once + second-session visibility kept); S4 tracker figures annotated in place; N5 → held graph pass; N6 → phase-8 forward note. Fix r1 prompt authored (`prompts/implementer/2026-08-14_phase7_fix_r1.md`). Fix r1: mirror kind gate, adopted reviewer probes, C8 second-session commit, literal C9 status, exactly-once/visible C10 seam, P-Z successor assertions, dead-branch/validator/whitespace cleanup; focused 82, concurrency 5 twice, phase-5 valuation/request 55, full non-E2E 2076/23/1 baseline-identical; all F1/M1–M7 mutations bite and were restored; DB head `be9dfe42a035`, economics tables 0, graph zero delta. Coordinator consumption (2026-08-14): checkpoint `bb233db` (10 files = declared perimeter exactly; handoff deposited after; `git diff bb233db..HEAD -- app/` empty); all FIVE final hashes re-verified byte-identical (4 production + the router's unchanged baseline); F1 gate, F7 dead-branch deletion, and N1 validator deletion confirmed by inspection (the gate is the exact specified `if`, history/audit blocks unchanged below it); ONE delegation outside the fix fence, declared and accepted — the F4 translation row extended `test_item_economics_requests.py` (+1) instead of a new file. Re-review r2 prompt authored (`prompts/reviewer/2026-08-14_phase7_rereview_r2.md`; probes R2-P1…R2-P7: r1-scenario re-runs, adoption-fidelity diff vs `probes/reviewer_r1/` sources, row-coverage map re-issue, M1–M7 hash-verified re-runs with M3/M4 row-1-only, +39 collection reconcile, rule 11½). **Re-review r2 (Claude Opus 5): APPROVED** — 0 blocking, 1 should-fix (routed), 3 notes (`handoffs/reviewer/2026-08-14_phase7_rereview_r2_handoff.md`). Perimeter verified (10 files exactly, `git diff bb233db..HEAD -- app/` empty, five hashes match, F4 delegation correct). **All 13 r1 findings closed**, each by re-running the arbiter that was green-or-absent in r1: B1 closed by the F1 kind gate (C5 row 7 passes on the shipped tree; the mutation reddens exactly it); B2 closed by adoption — line-by-line diff vs the preserved `probes/reviewer_r1/` sources shows **zero assertions weakened or deleted**, integration nodes 4 → 42, ids name authority rows; B3 closed on a real two-session harness that BITES (M10); S1–S5 and N1–N4/N7/N8 all closed, with M8/M9/M10 (reviewer-authored) proving the NEW rows bite. **Five of seven declared mutant hashes reproduce byte-for-byte** (F1, M1–M5, M7; M6 differs in mutant text only), each reddening exactly its named row — M3/M4 row 1 only per chain. Numbers clean at last (P-L, third round): full non-E2E **2076/23/1** as declared, failure set byte-identical to the phase-1 list, **+39 reconciled exactly** (+38 integration nodes + 1 translation row); concurrency subset 5 twice; phase-5 surface 55; ruff clean; DB at head; economics tables 0 rows before and after; graph READ-ONLY zero delta (166/239, 52 pending, rev `0a71061…`). **Open and routed:** R2-S1 → closeout commit (the P-AB enumeration in the helper docstring and plan F1 lists "the audit row" as `kind`-gated; it is not — `audit()` runs on every path under a caller-supplied `audit_event`, and §6.4 registers `.projected`/`.promoted`; the gated set is five: chain-close, `committed_at`, mirror, history, event); R2-N1 anchor drift +4 for 13 graph items → the held graph pass (with N5); R2-N2 C10 seam non-vacuity → phase 8 (with N6); R2-N3 cosmetic. **Coordinator CLOSEOUT (2026-08-14):** R2-S1 applied — "the audit row" dropped from the helper docstring (line-shift +1: spans below the docstring now +5 vs the pre-fix graph records) and plan F1 corrected with the every-path clause (new `commit_item_cost_evaluation.py` sha256 recorded in the closeout commit); §9 += P-AB companion clause (enumerations read off the parameter's occurrences), adoption-fidelity rule, P-I 9th ext (reproducible mutant phrasing — 5/7 mutants reproduced byte-for-byte, first re-derivable ledger), P-S template (delete unbuildable directions, keep the buildable arbiter); §10 baseline → 2076/23/1 = 2099 selected, head unchanged `be9dfe42a035`; 10 artifacts archived to `archive/plan_7/` (probes/reviewer_r1 kept in place — the adoption-fidelity rule's named path); no residue purge owed (economics tables 0 before/after both reviewer sessions). Post-approval graph pass follows the closeout commit (52 pending: 50 phase-7 + the 2 phase-6 re-records; R2-N1 spans service + N5 type question in hand) |
| 8 | Status & results | `plans/phase_8_status_results.md` | ⚑ (rows 9,26–30,34) | **APPROVED** | 2026-08-15 | Codex / implementation-executor; coordinator; Codex (r1b); reviewer r1 (Claude Opus 5); Codex (fix r1); re-reviewer r2 (Claude Opus 5); Codex (fix r2); re-reviewer r3 (Claude Opus 5) | Test-side fix r2 complete: H1–H6 satisfied, H7 scope held with zero production edits, zero migrations, and zero Architecture Graph writes. Focused phase scope 146 passed; H3 pair 2 passed on each of 10 consecutive runs; full non-E2E 2138 passed / 23 established failures / 1 deselected with the sorted failure set byte-identical to the phase-1 baseline; DB head `c1d2e3f4a5b6`; zero item-economics evaluations/results and zero `process_item_cost_result` execution tasks; mutation ledger complete with MX1, MX2, M1, M2, M3, and M17 each reddening its named row, all temporary production mutations reverted, and final hashes recorded in the phase Review log and fix handoff. **Coordinator consumption (2026-08-15):** checkpoint `6988364` = exactly 4 test files + 2 docs (H7 held: ZERO production edits; diff-app empty; the three mutation-target production files hash back to their r2 pre-images byte-identically; probe final `7df683f7…` matches). **MX1/MX2 mutant hashes REPRODUCE the r2 reviewer's bytes exactly** (P-I 9th hard field satisfied — the ledger carries per-row mutant hashes this time); H-list fully ticked (row-list tick rule's first application: clean); H3 2×10 flake-free; suite 2138/23/1 with sorted IDs = phase-1 set. Re-review r3 prompt authored (`prompts/reviewer/2026-08-15_phase8_rereview_r3.md` — THIN delta: R3-P1…P5, MX re-runs from the reviewer's own bytes, the two newly-authored H4/H5 fixtures read hard, flake 10× re-check; approval expected)  **Re-review r3: APPROVED — 0 blocking / 0 should-fix / 4 notes, 0 owner cards** (`handoffs/reviewer/2026-08-15_phase8_rereview_r3_handoff.md`). Both r2 blockers CLOSED and both r2-added mutations now BITE from the reviewer's own bytes (MX1 `assert 150 == 120`, MX2 `assert 'ok' == 'infeasible'`); the three r2 should-fixes closed. C7 now enumerates all TWELVE shipped members with BOTH producers real (10 readiness ids + `P-V-infeasible`/`P-V-ok` through `get_task_budget_status`). Reviewer-added MX1b/MX1c/MX3 also bite (each excluded column discriminates independently; the `infeasible` ⇒ null-percent clause is arbitrated on the production path). S1 flake structurally dead (zero order-dependent picks; 10x2 green; M1/M2 bite 62/62 row-executions across focused and wide scope). Perimeter exact: app/ byte-identical to `6988364`, all 7 declared hashes reproduce, 5 production files restored. Numbers re-derived: 2138/23/1, failure set BYTE-identical to the phase-1 list, collection net 0 (-2 serializer / +2 integration). DB at head, zero item-economics residue, graph read-only 172/254 rev `c74eb913…` 21 pending. Notes N1-N4 carried forward. **Coordinator CLOSEOUT (2026-08-15):** reviewer perimeter exact (3 docs; app/ byte-identical to `6988364`). Phase 8 APPROVED across five rounds (r1 incomplete→r1b migration→review r1→fix r1→re-review r2→fix r2→re-review r3): both r2 survivors killed on the reviewer's own bytes, FOUR reviewer-added r3 mutations also bite (MX1b/MX1c distinct-value leak-naming, MX3 null-percent, G7-probe on a different route) — the fix exceeded its acceptance condition; C7 finally enumerates all 12 members on real producers (A1 satisfied for the first time). §9 += 4 r3 rules (structural-filter, distinct-values, scenario-fixture, repair-adversarial); §10 baseline → **2138/23/1 = 2161 selected**, head `c1d2e3f4a5b6`; N1/N2/N4 → phase-9 forward notes (N1 = the compiled-statement structural arbiter, first phase touching the status queries); 16 artifacts → `archive/plan_8/` (probes/reviewer_r1_phase8 kept — named path); no purge owed (zero economics residue verified three sessions running; the 1 valuation + 1 basis row are pre-existing non-orphaned dev data). Post-approval graph pass follows the closeout commit (21 pending + the enum-migration mapping + 3 A16 discrepancy filings + the status queries' node-type + the 1 stale node r2/r3 observed) |
| 8B | Inline item prices at task creation | `plans/phase_8b_inline_task_prices.md` | ⚑ (R18-1 — owner scope addition 2026-08-15) | **APPROVED** | 2026-08-15 | coordinator (round 18); Codex implementer r1; reviewer r1 (Claude); Codex (fix r2); reviewer r2 (Claude) | §7B.6 implemented: optional inline valuation trio with currency-iff-amount validation; new-item valuation v1 / matched-item current-valuation refusal; deleted-or-superseded-only existing items advance the chain; legacy keys remain `ITEM_MONEY_MOVED`; task-route README and router boundary updated. The 21-node phase harness covers C1–C6, owning C4 teardown, validator order, and B8 non-vacuity; the retained bridge parametrization adds 24 nodes. Focused phase plus retention: 66 passed. Full non-E2E: 2183 passed / 23 established failures / 1 deselected (2207 collected), sorted failure IDs baseline-identical; ruff clean; DB `c1d2e3f4a5b6` head; no migration. Implementer handoff: `handoffs/implementer/2026-08-15_phase8b_implement_r1_handoff.md`. **Coordinator consumption (2026-08-15):** checkpoint `513856d` (9 files = fence exactly; handoff after; diff-app+archgraph empty). ALL SEVEN final hashes recomputed byte-identical; the ledger meets the full endgame bar UNPROMPTED (expected-red ids pre-declared, per-row mutant AND restored hashes, zero deferrals — the first r1 in the project to do so); mechanism confirmed by inspection (single write site :317-353, refusal + registered identity, audit call, validator order legacy-first); graph delta = the NEW command-task-create node + 4 edges (5 pending HELD, rev `53fdbc78…`). Review r1 prompt authored (`prompts/reviewer/2026-08-15_phase8b_review_r1.md`; probes P8B-1…P8B-7 — full-depth on the small surface: 5-mutation re-run from declared bytes, the refusal predicate read hard incl. the superseded-only shape (a missing INV-V1 conjunct silently flips branch B to A), effect-set discipline, bridge unsoftened, +45 reconcile, the new node's unverified anchors) | **Review r1 (Claude): CHANGES_REQUESTED** — 2 should-fix, 3 notes, ZERO blocking; the mechanism is correct on every branch re-derived, both findings are test-integrity. Independently re-verified: seven hashes byte-identical, tree clean, suite 2183/23/1 (2207 collected) with sorted failure IDs byte-identical to the phase-1 list, +45 reconciled by `--collect-only` (21 phase + 24 bridge), focused 66, ruff clean, DB at head, zero `phase8b` residue after restoration; all five declared mutations re-run with every named expected-red observed, and FOUR of five mutant hashes reproduced byte-identically from their descriptions. **S1** — the `superseded_at IS NULL` conjunct of the refusal predicate is uncovered: deleting it (reviewer M6) leaves all 66 nodes green, and B4's second branch-B sub-row (superseded-only) was never shipped; the state is reachable via `set_item_valuation`×2 + `delete_item_valuation`, and a reviewer probe proves shipped code accepts correctly there while M6 wrongly refuses. **S2** — C4 rows 2/3 violate rule 11½: their `finally` dereferences `workspace.client_id`/`user.client_id` on ORM instances expired by the rollback, so cleanup raises before any DELETE and every red run leaks committed rows (8 leaked during this review, removed by hand); row 1 already has the correct shape (locals captured before the `try`). Notes: the handoff's "teardown removed them" claim is not reproducible (N1); the pending `command-task-create` node's anchors are partly wrong — node span starts at the module `logger` not the function, and the `writes_to table-task` span excludes the add/flush it claims (N2, corrected spans in the plan Review log, human-adjudicated); M1's "1 failure" was the named node only, not the focused scope (N3). §7B.5/B8(ii) additionally verified LIVE (savepoint rollback keeps the valuation). Reviewer handoff: `handoffs/reviewer/2026-08-15_phase8b_review_r1_handoff.md`  **Coordinator (post-review, 2026-08-15):** reviewer perimeter exact (2 docs + handoff; app/+archgraph = `513856d` blobs). Production verified correct on EVERY branch incl. two no-test-reaches shapes; §7B.5 survival property observed LIVE; effect set diffed vs the PUT path (exact); M2–M5 byte-reproduce, M6–M9 reviewer-original conjunct mapping (M6 = the S1 gap: superseded_at conjunct unguarded, correct in code; M8 workspace conjunct redundant-by-construction, not a finding; M9 proves C5.3 non-vacuous). §9 += per-alternative rule, rule-11½ capture-locals mode, P-I 10th ext (line-pinned deletion boundaries + scoped observed-red), birth-anchor rule. Plan F1–F4 GOVERNING (F1 superseded-only row via 3 production commands + M6 expected-red; F2 capture-locals + the M2-rerun zero-residue proof; F3 ledger discipline; F4 corrected spans held for the graph pass). Fix r2 prompt authored (`prompts/implementer/2026-08-15_phase8b_fix_r2.md` — ONE test file, two items)  **Fix r2 (Codex, 2026-08-15):** F1 added C4 row 4 through the three production valuation commands; M6 deletion at `create_task.py:331` reddened exactly that row over the full phase-file scope (1 failed / 21 passed), mutant SHA-256 `98dc2c252e8f5bdac1ea7ecc5aeff0391fd6fd081f684d45dbf86ada718174bd`, restored production hash `e9c2ccc1bfda4c133149195e530977860eaefba31508b30e492a42bd6e41d547`. F2 captured rollback-safe locals in C4 rows 2/3; M2 invert at `create_task.py:337` (`f0776418c7cdc77faf76907bc47545ce70d244106e35d6e88ba9f09940cb2f95`) reddened exactly row 2 in its one-row scope; post-run state query returned zero `phase8b` workspaces/users/categories/audits/valuations. Clean focused phase scope: 22 passed; full non-E2E: 2184 passed / 23 established failures / 1 deselected; DB `c1d2e3f4a5b6` head; ruff clean; architecture graph read-only, zero delta.  Fix r2 (Codex, `4369a27`) CONSUMED 2026-08-15: ONE test file (+117/-13; final `12c6ad5b…`), zero production edits (create_task byte-identical to implement-r1 final — coordinator recomputed); F1 row 4 via the 3-command recipe with M6 line-331-pinned biting exactly its row (mutant `98dc2c25…`); F2 capture-locals + the M2-under-red zero-residue state query (coordinator re-ran it: 0/0); F3 line-pinned scoped ledger (P-I 10th ext satisfied at first use); suite 2184/23/1 (+1 exactly); graph zero delta (5 pending held). Re-review r2 prompt authored (`prompts/reviewer/2026-08-15_phase8b_rereview_r2.md` — thinnest delta of the project; approval expected) | **Re-review r2 (Claude): APPROVED — phase 8B is DONE.** Zero findings. Perimeter verified: exactly one `app/` path changed in the fix cycle (the phase test file); all five other production/graph artifacts recomputed byte-identical to their implement-r1 finals — zero production change as declared. **S1 dead:** C4 row 4 seeds through the three production commands, asserts the no-current-valuation pre-state at DB truth, and pins rows 0/1 to their ORIGINAL client_ids (stronger than the reviewer probe it came from); M6 re-run reproduced mutant `98dc2c25…` **byte-identically** and reddened exactly that row, 1 failed / 21 passed over the full phase-file scope. **S2 dead:** rows 2/3/4 all pass plain locals to cleanup; the M2 inversion that leaked 2→8 workspaces in r1 was re-run here — all four committing C4 rows redden and the state query returns 0/0/0/0/0. Numbers re-measured in foreground: 2184 / 23 (sorted IDs `diff`-empty vs the phase-1 baseline) / 1, collection 2208 = +1 reconciled exactly by the one new node (phase file 21→22), focused phase+bridge 67, ruff clean, DB at head, graph read-only 174/260 rev `53fdbc78…` zero delta, tree byte-identical to `4369a27` at close. **P-I 10th ext validated at first use** — both fix mutations line-pinned, both hashes reproduced byte-identically from their descriptions alone. Carry-forward: the 5 pending graph items + F4's corrected spans → the coordinator's post-approval human pass; the phase-7 `item_valuations` residue row → the existing rule-11½ maintenance record. Reviewer handoff: `handoffs/reviewer/2026-08-15_phase8b_rereview_r2_handoff.md`  **Re-review r2 (Claude): APPROVED — 0/0/0.** Both r1 should-fixes dead at the seam: M6 reproduced byte-identically and reddens exactly the new superseded-only row (chain-grew proven by ROW IDENTITY, stronger than the r1 probe); the M2-under-red run that leaked 8 workspaces in r1 now leaves ZERO (the 11½ companion check validated). Numbers all re-measured: 2184/23/1 (2208 collected, +1 exact), failure diff empty, focused 67, ruff clean, graph zero delta, tree = checkpoint blobs. P-I 10th ext two-for-two at first use. **Coordinator CLOSEOUT (2026-08-15):** §10 baseline → 2184/23/1 = 2207 selected (head unchanged `c1d2e3f4a5b6`); §9 rule-11½ companion check added; 10 artifacts → archive/plan_8b/; no purge owed (zero residue verified by reviewer AND coordinator; the phase-7 ival row rides the existing maintenance record). Post-approval graph pass follows (5 pending w/ F4 spans). Phase-9 HOLD LIFTS |
| 9 | Living docs & drift routing | `plans/phase_9_docs_and_drift.md` | waivable (no S1/S2 mechanism; docs only) | **APPROVED** | 2026-08-15 | Claude Opus 5 / implementation-executor (implement r1); reviewer r1 (Claude Opus 5) | living-docs page, §2.6 + D-1…D-4 landing spots  Projection r0 prompt authored 2026-08-15 (`prompts/reviewer/2026-08-15_phase9_projection_r0.md`; the LAST phase — axes: forward-note census across all eight phases' archives, docs-page accuracy arbiters (registry-grep harness), the one weighty code item (r3-N1 structural compiled-statement filter arbiter), deploy-ordering doc placement, the COMPLETE enumerable code-touch list as a hard fence, v1-closure shape incl. post-v1 handoffs)  **HOLD LIFTED 2026-08-15** (8B APPROVED+closed): the projection prompt's banner now carries the superseding env facts (baseline 2184/23/1 = 2207 selected; graph 174/260 all confirmed rev `452befdb…`; head unchanged) and the R18-2 + 8B-routed handoff scope. READY TO RUN — the last gate before v1  **Projection r0 (Claude Opus 5): AMENDMENTS_REQUIRED** — 5 B / 15 S / 8 L + the 32-row forward-note CENSUS (5 UNROUTED caught: the tables index, the eleven annotations, N10, N5-payload, the two contract amendments; 2 upstream notes factually WRONG: the routed C5 repair reddens the test, N14 named :176 for a defect at :179; 1 undischargeable: phase-2 N4 vs an applied migration). Blockers: the living-docs deliverable violated its own cited contract (FOLDER + docs/README row, not a flat file); C1 had no harness (no repo test reads .md); r3-N1 unpinned; the Goal fence contradicted eight routed code items; models/tables/README never routed while its frontend MIRROR was. Coordinator routed ALL rows same day: owner card → **R19-1 TWO handoffs, all 23 endpoints** (operational 10 + configuration 13, split for the owner's two-stage frontend build); §6.5 folder correction; §11 += F14's four uncovered filter sites; squash seed += Finding 8; plan GOVERNING block **P1–P22** (incl. decisions: N14 TAKEN at :179; annotations TAKEN; money-audience node BUILT (P14 — satisfies §13's archgraph clause); deploy line → docs/deploy; Application_contracts + frontend mirrors OWNED with stop-and-report access guards; N4 WONTFIX; C4's hand-written 23-row arbiter; the PUT-table repair + banner fix). State → PROMPT_READY; implementer prompt authored (`prompts/implementer/2026-08-15_phase9_implement_r1.md`, intended for a Claude-model session per the owner's choice)  **Implement r1 (2026-08-15): IMPLEMENTED.** GOVERNING block P1-P22 discharged - living-docs FOLDER (`docs/domains/item_economics/` x4 + the `docs/README.md` row), the TWO frontend handoffs (13 configuration + 10 operational = all 23 routes), P3's three-site compiled-statement filter arbiter (3/3 named mutations executed, per-site red only, mutant+restored hashes in the Review log), P4's full code allow-list (11 annotations, 4B N3 :39/:49, the docstring, the downgrade-proxy regex, N14 :179, the C5 repair, the two-line format edit), the README/prefix-map/mirror/contract batch, and P20's two contract amendments. Suite **2249/23/1 = 2272 selected**, failure set byte-identical to the phase-1 baseline (sorted diff empty), +65 reconciled exactly (8+3+4+50). ruff clean on all 15 touched Python files; DB at head `c1d2e3f4a5b6` (no migration ran). Archgraph **+1 node** (`decision-money-audience-admin-manager-only`) -> 175/260, rev `7dcdb9b0...`, 1 pending, 1 stale (`endpoint-item-economics-status`'s anchor, caused by 4B N3's own edit - filed for the maintenance channel). N4 closed WONTFIX with rationale; 8 drift items filed unfixed as out of fence. Handoff: `handoffs/implementer/2026-08-15_phase9_implement_r1_handoff.md`  **Implement r1 (Claude Opus 5, `4b648c0`) CONSUMED 2026-08-15:** P1–P22 discharged in full — the docs folder + docs/README row, BOTH handoffs (all 23 endpoints), FOUR new arbiter files (65 nodes: 8 C1 + 3 P3 + 4 C4 + 50 handoff-accuracy incl. exact-set heading equality and shipped-key comparison), the nine-item code allow-list, the README/prefix/mirror/contracts batch, the P14 decision node. Suite **2249/23/1 = 2272 selected** (+65 reconciled exactly); 3 named mutations + **4 SELF-CHOSEN probes** (the new arbiters reached review falsified — first implementer to do this unprompted), all line-pinned with hash pairs; 8 drift items FILED not fixed; 10 judgment calls declared. Coordinator verification: 10 hashes byte-identical ACROSS THREE REPOS (frontend = exactly the 2 declared uncommitted files — owner decides the commit at closeout; Application_contracts hash-verified, gitignored); checkpoint amend noted (`71616af` pre-amend object, trees differ by 2 planning lines); graph 175/260, 1 pending (the money-audience node, HELD) + 1 stale (config-status link content-drift — the known re-accept repair, closeout). Review r1 prompt authored (`prompts/reviewer/2026-08-15_phase9_review_r1.md`; P9R-1…P9R-7 — the PROSE read is the heart; the 65-node suite read hard; the eleven annotations proven INERT via alembic check; the ten judgment calls verified)  **Review r1 (Claude Opus 5): APPROVED** — 0 blocking / 4 should-fix / 7 notes / 1 owner card. All four domain-doc files and BOTH handoffs read in full; every load-bearing sentence re-derived against the code (vocabulary + branch structure — P16/JC1 correct, §11A.4 group 1 IS a branch condition so P16 and F20 do not conflict; the two precedence tuples; the handler admission table total over all eight task states; the four emission points; the event and audit vocabulary; EVERY payload key catalog field-by-field vs the serializers; the 23 routes and the single four-role gate; §11A.2's eight-endpoint census; the exact bridge and inline-refusal messages; the savepoint semantics behind the rollback and the six silent auto-commit outcomes; both contract divergences vs the real canonical text). Numbers re-measured: 2249/23/1 = 2272, failure set diffed vs the phase-1 recorded 23-item list — EMPTY; +65 counted per file; **`alembic check` = exactly three pre-existing drifts, no fourth ⇒ the eleven annotations proven INERT**; ruff clean on 15; DB at head; perimeter exact (34 = 33 declared + .archgraph), nothing outside P4's allow-list. Mutations: M1/M2/M3 reproduce with mutant AND restored hashes matching and zero collateral; P-c exact; **P-d reddens THREE nodes not the declared two (N1, charter L8)**; FIVE reviewer-added probes on the unfalsified arbiters, incl. **R-h** proving phase-2 N8's regex live (raw-SQL DROP TYPE reddens the proxy) and **R-g1/R-g2** proving C1 bites a reword and tolerates a rewrap. The identity census verified BY HAND = 30 literal + 6 composed, exactly §6.4. **4 should-fix, none in the six fenced documents:** S1 the frontend mirror's four `item_currency` rows say Required=Yes against `| None = None` and the twelve annotations lack the non-null clarifier the backend README carries — the mirror files are UNCOMMITTED, so this is a zero-cost closeout fix (**owner card 1**); S2 the lifetime endpoint serializes `terms=[]` per episode, documented nowhere; S3 `05_errors_local.md:114`'s pydantic rule is unqualified and contradicts ~20 request modules; S4 `item_models.md` is self-contradicting after P13's fenced rewrite (implementer's fence discipline right, the fence wrong — L1). All 8 filed drift items independently VERIFIED REAL. All P22 ticks hold except N7 (the squash seed has no consolidated home — coordinator's closeout). Graph read-only, 1 pending + 1 stale untouched for human adjudication; P14's four evidence spans re-read and accurate. Lessons L1–L4. Handoff: `handoffs/reviewer/2026-08-15_phase9_review_r1_handoff.md`  **Review r1 (Claude Opus 5): APPROVED — 0 blocking / 4 should-fix / 7 notes, 1 owner card.** Every document read IN FULL sentence-by-sentence (nine files, 2,680 lines — the prose-verification record enumerates them); all 3 named mutations reproduce byte-for-byte + P-c/P-d re-run + FIVE reviewer-added probes (incl. R-g1/R-g2 proving the docs alarm distinguishes a reword from a rewrap, and R-h proving phase-2 N8's regex is live); the identity census hand-counted exact (30+6); the eleven annotations proven INERT (alembic check = exactly the 3 pre-existing drifts); all 8 filed drift items verified real; all P22 ticks hold. **Coordinator CLOSEOUT — v1 CLOSES (2026-08-15):** owner card 1 → FIX FIRST THEN COMMIT (S1 applied to all four item_currency rows + per-table clarifiers; frontend repo committed `1793056b` — first attempt matched only 3 of 4 row shapes and was amended, recorded); S2 applied (terms:[] sentences in api.md + handoff §5.2 — all 58 doc arbiters re-run green); S3 applied (the branch-on qualifier in 05_errors_local.md); S4 + drift 5 → the post-v1 Application_contracts follow-up pass; N7 RESOLVED — the squash seed's consolidated home EXISTS at `docs/architecture/under_construction/implementation/squash_migrations/research_context.md` (Findings 1–8; the reviewer looked inside item_cost_calculation only); N6 → §11; drift 1 + the pending P14 node → the graph pass below; drifts 2/6/7/8 next-touch, 3/4 → the routers-README follow-up pass; §9 += the four L-rules (fence-contradiction, mirror-column, direction-declared, contract-vs-tree); §10 baseline → **2249/23/1 = 2272 selected — v1 CLOSURE baseline**; 6 artifacts → archive/plan_9/ |
...
- **Audit event vocabulary (registry-authored 2026-08-12, phase-4 projection S7)** —
  ...
  **`item_cost_evaluation.committed` / `.projected` / `.promoted` /
  `.deleted`** (phase 7, registered 2026-08-14 per projection D8 — `.deleted`
  covers projection deletion only; committed rows are never deletable, INV-E2.
  The auto path writes `.committed` too, attributed to the task creator).
- **Commit-path status→identity mapping (registered 2026-08-14, phase-7
  projection D1)** — every commit-path refusal is a translation from the
  phase-5 resolver's `EconomicsStatusEnum` to an error identity; this table is
  THE mapping (never re-derived per command). All `ValidationError`; C7's rows
  exercise the **resolver route** (the resolver gate runs before the
  calculator, per P-F's "nothing is written before the calculator succeeds" —
  the calculator's own copies of the last three identities stay as armor):

  | `EconomicsStatusEnum` | commit-path identity |
  |---|---|
  | `ITEM_MISSING_MAJOR_CATEGORY` | **`ITEM_COST_ITEM_MISSING_MAJOR_CATEGORY`** (registered here — resolves the 4B N-c forward; message names the item) |
  | `NOT_CONFIGURED_NO_COST_GROUP` | `ITEM_COST_NO_COST_GROUP` |
  | `NOT_CONFIGURED_AMBIGUOUS_COST_GROUP` | `ITEM_COST_AMBIGUOUS_COST_GROUP` (message: count + ids built from the group rows the command already loaded, filtered by category — message construction, NOT a second selection derivation; `EconomicsSelection` stays frozen as shipped — projection D12) |
  | `NOT_CONFIGURED_NO_BASIS_VERSION` | `ITEM_COST_NO_BASIS_VERSION` |
  | `NOT_CONFIGURED_NO_COST_MODEL_VERSION` | `ITEM_COST_NO_COST_MODEL_VERSION` |
  | `ITEM_UNVALUED` | `ITEM_COST_ITEM_UNVALUED` |
  | `ITEM_MISSING_EXPECTED_PRICE` | `ITEM_COST_EXPECTED_PRICE_REQUIRED` |
  | `ITEM_MISSING_PURCHASE_COST` | `ITEM_COST_PURCHASE_COST_REQUIRED` |
  | `CURRENCY_MISMATCH` | `ITEM_COST_CURRENCY_MISMATCH` |
  | `NOT_EVALUATED` | *(proceed — no identity)* |

- **`_common.py` conflict translation, phase-7 rows (projection D2, decided
  2026-08-14):** `INDEX_IDENTITIES` gains
  `uix_item_cost_evaluations_current` → `ITEM_COST_CONCURRENT_COMMIT`; the
  uniform conflict sentence (`"{identity}: configuration conflicts with an
  existing row"`, phase-4 N4) **stands** — the identity token is the asserted
  carrier and per-identity sentences are NOT authorized. The commit command,
  `promote_item_cost_projection`, and the auto path all route their chain
  INSERTs through the same translation helper, or the identity exists on one
  path only.
- Inline birth pricing (registry-authored 2026-08-15, phase-8B projection
  L11 adapted for the owner's branch-B answer R18-3):
  **`ITEM_COST_INLINE_PRICE_ON_PRICED_ITEM`** — `ValidationError`, raised
  when task creation carries the inline trio for a matched existing item
  that has a CURRENT valuation (INV-V1 predicate); message names the
  matched item's `client_id` and points at the valuation endpoint as the
  price-change surface. Single-path (application pre-check only; P-S: no
  DB arbiter exists — discharged by the pre-check row plus this note,
  never an invented fixture).
- API bridge (§10A.3, carrier CORRECTED R14-1): the repo's
  `beyo_manager.errors.validation.ValidationError` (a `DomainError` — NEVER a
  pydantic-side `ValueError`, whose message the parse helpers mangle) with the
  exact full message
  `ITEM_MONEY_MOVED: item money fields moved to the item-valuation endpoint`,
  surfacing as `{"error": "ITEM_MONEY_MOVED: …", "ok": false}` at 422.
- Migration pre-flight P1/P2/**P3** (§10A.2 as amended R14-3): `RuntimeError`
  aborting `upgrade` with a row report — never a DomainError (no request
  context). P3 = amount present with the item's `created_by_id` NULL.
...
- **Workspace event:** `item_economics:evaluation-committed` (matches
  ...
  on the auto path it rides `create_task`'s `pending_events`, appended only
  after the savepoint exits normally — R16-4).
- **Phase-8B addition (R18-1, registered 2026-08-15):**
  `FindOrCreateItemInput` gains the valuation trio —
  `expected_sale_price_minor`, `purchase_cost_minor`, `currency` — mirroring
  `ItemValuationRequest` (`requests/__init__.py:116-121`) exactly; the
  legacy names stay rejected (§10A.3 unchanged). The birth write consumes
  `write_item_valuation_chain_in_session` and the registered
  `item_valuation.created` audit event; the existing-item refusal identity
  is PROPOSED by the 8B cycle before use (4B N-c pattern).
- **Phase-7 additions (projection r0 D10/D13/D15/D18/D19, registered
  2026-08-14):**
  - `_commit_item_cost_evaluation_in_session(...)` — the commit procedure as an
    explicit-argument helper in `commit_item_cost_evaluation.py`, returning its
    pending events; the command is a thin wrapper over it;
    `promote_item_cost_projection` and `create_task`'s auto path consume the
    SAME helper (D10 — the only shape compatible with the subordinate-command
    event rule). **`auto_commit_item_cost_evaluation_in_session(...)`** (same
    module — implementer addition, registered post-hoc by the coordinator
    2026-08-14 at implement-r1 consumption): the auto path's entry — holds the
    §7B.5 pre-check (resolver `NOT_EVALUATED` + active PRIMARY) and delegates
    to the core helper; reviewer verifies it holds NOTHING else.
  - `_common.py` gains (extracted from `set_item_valuation.py`, phase-5 call
    sites re-pointed — D19; **P-Z binds**: shared-machinery scope exception
    carries before/after property tests): the valuation-chain S1→S2→S3 writer
    (currently inline at `set_item_valuation.py:128-159`) and the
    workspace-config loader (`_load_preview_inputs`). Never duplicated — a
    duplicated chain writer is how the two drift.
  - Rate snapshot provenance (D13, decided: **recompute**):
    `cost_per_worker_minute_minor_snapshot` is written from
    `calculate_cost_per_worker_minute` over the three snapshot inputs, never
    copied from `basis.cost_per_worker_minute_minor` — P-F ("snapshots are
    written only from calculator outputs"), and §6A.11's rederive theorem
    requires it. The two agree today because R11-1 canonicalizes request
    numerics to column scale before deriving; a criterion needs a hand-written
    basis row where persisted ≠ derived to tell the routes apart (P-Q).
  - Serializers (D18): `serialize_item_cost_evaluation` (committed +
    projection rows; carries `kind`, `label`, figures, snapshots, provenance
    ids, `committed_at`/`superseded_at`, `promoted_from_id`) and
    `serialize_item_cost_evaluation_term` (drill-down). Request models in
    `requests/__init__.py`: `CommitItemCostEvaluationRequest` (optional
    `expected_sale_price_minor` / `purchase_cost_minor` overrides — never a
    currency, §6A.9; optional `label`), `CreateItemCostProjectionRequest`
    (`source`: `"committed" | "projection" | "scratch"`; `source_projection_id`
    required iff `source == "projection"`; same optional overrides + `label`),
    promotion/deletion take no body. `label` is accepted on commits and
    projections, optional both places, stored verbatim (nullable String(255)).
  - Evaluations read (D15, all four axes pinned): route
    `GET /tasks/<task_client_id>/evaluations` returns
    `{"evaluations": [...], "projections": [...]}` — separate keys (the R13-1
    principle: committed and speculative figures never merge into one list);
    committed chain ordered `committed_at DESC, client_id DESC` (current row
    first), projections ordered `created_at DESC, client_id DESC` (R13-2's
    idiom); **unpaginated** (both sets are bounded per task; the limit+1 idiom
    is for unbounded lists); each row carries `terms` ordered
    `created_at ASC, client_id ASC` (terms have no ordinal column and per-row
    `now()` defaults — without the explicit ORDER BY the payload is
    non-deterministic).
  - The rederive marker's consumer (D16, decided): **phase 7's evaluations
    read owns it** — it is the read that returns snapshots. Each returned
    evaluation runs `rederive` on the loaded rows; a `REDERIVE_MISMATCH`
    entry is logged at ERROR ("integrity check failed" copy — NEVER "data
    corruption", N15) and the read still renders (R10-2 homogeneous payload).
    Phase 8's status query inherits the same discipline.
  - Auto-path skip-line vocabulary (review-r1 N3, decided 2026-08-14): the
    `item_economics.auto_commit_skipped` line's `status` field ranges over
    `EconomicsStatusEnum` values **∪ the literal `no_primary_item`** — the
    no-item state is task-shaped, not item-readiness, so NO enum member is
    added; the literal is registered here and asserted by C9's tenth row.
- **Living docs (CORRECTED at phase-9 projection B1, 2026-08-15):** the
  `23_documentation` contract mandates a FOLDER —
  `docs/domains/item_economics/` with `README.md`, `api.md`, `events.md`
  (the domain emits an event, so it is required), `states.md` (the
  twelve-status vocabulary + the evaluation chain) — PLUS a domain-map
  row in `docs/README.md` (contract-mandatory; the flat-file form the
  registry carried since round 3 violated the cited contract; precedent
  `docs/domains/worker_shifts/`).
...
- **P-E (HC-3):** no phase modifies `step_state_records` writers, the concurrency
  ...
  **AMENDED at phase-8 projection r0 (L4, 2026-08-14):** the reopen hook forces
  `maybe_reopen_task_to_working` to become async and take a session — the fence
  ADMITS exactly the mechanical adaptations that signature change forces: the
  awaited call site in `add_task_steps.py:182` (no logic change; the returned
  bool's consumption unchanged) and the two sync call updates in
  `tests/unit/test_task_state_transitions.py`. Nothing else in the execution
  path, still.
...
- **P-J second extension (phase-5 re-review-r2 L1):** when a criterion states a
  property over a **module set**, the assertion QUANTIFIES over that set — a
  test that constructs the set and then asserts about one member has not
  discharged it (the shipped L15 guard caught "resolver call replaced" and
  missed "second reader added", the only failure mode a structural row exists
  for).
- **P-V numbering rule (r2 L2):** a plan giving a parametrize-id example quotes
  the authority's CURRENT numbering — L4's own example was pre-round-12 and the
  implementation shipped a hybrid scheme, costing the re-review a full re-audit
  where one `--collect-only` should have sufficed.
- **P-I seventh extension (r2 L3/N4):** a REUSED mutation name carries its
  mutant hash beside each red set — two different M4 mutants shipped under one
  label and the ledger's red set belonged to the one that did not exercise the
  row under review.
- **Reviewer practice (r2 L4):** suite runs are foreground, or probes wait — a
  background suite run overlapping the reviewer's own mutants produced a
  phantom 24th failure (self-caught, corrected, recorded).
- **P-J third extension (phase-5 re-review-r3 L1):** a criterion mandating a
  property over a DISCOVERED set also mandates a non-vacuity row — `assert
  unmediated == {}` over an empty walk is a silent pass; the arbiter must be
  designed, not accidental.
- **Scope-predicate rule (r3 L2):** when a plan writes a glob
  (`services/**/item_economics/`), the test uses the glob's semantics
  (`"item_economics" in path.parts`), never a convenient approximation
  (immediate-parent checks missed exactly one file).
- **P-J fourth extension (phase-6 review L1 — production guards):** a
  criterion mandating an in-migration post-condition owes the mutation that
  must ABORT it — a guard whose two sides are built from the same
  construction is a tautology, and there is no second run to catch it (B1: a
  partial copy exited 0 and the next migration dropped the columns).
- **P-V third extension (phase-6 review L2):** a parametrize id is a label,
  not a probe — when the parameter is never CONSUMED by the test body, N rows
  are one row; an enumerated census criterion requires each row's EXPRESSION
  to differ.
- **Projection practice (phase-6 review L3):** the decidability pass REJECTS a
  clause whose stated intent and verbatim predicate select different rows —
  "follow it verbatim" cannot discharge a self-contradicting clause (B2:
  round-14's folded predicate re-valued the prices its heading protected).
- **Row-report rule (phase-6 review L4):** a refusal criterion naming a row
  report owes an assertion on the report's CONTENTS (the offending ids), and
  identity tokens are asserted per-refusal-class — one message carrying every
  identity satisfies any substring assertion vacuously.
- **End-state counting rule (phase-6 review L5):** a criterion counting users
  of a shared type is written against the end state the phase actually
  produces — D9's "2 → 1" forgot the journal's own enum snapshot column
  (head has 2 users; 1 arrives only with the squash).
- **P-I eighth extension (phase-6 re-review-r2 L1):** a fix cycle's own RECORD
  is evidence and gets verified like any other claim — a fix criterion naming
  N sub-items owes a per-sub-item observation, never a summary sentence (the
  r2 round's only finding was a record asserting a closure that did not
  happen).
- **Precedence-arbitration rule (r2 L2):** an `OR`ed structural assertion
  arbitrates its precedence — re-run with one conjunct falsified and confirm
  the count DROPS; a query that keeps counting is mis-parenthesised.
- **Surviving-conjunct rule (r2 L3):** when a fix replaces a predicate, ask
  per surviving clause whether any case still depends on it — rule 2's
  non-vacuity question applied to production predicates.
...
- **P-T third extension (phase-7 review L1):** a row-lock criterion names its
  counterparty's lock **MODE**, not just its identity — FK `KEY SHARE` is
  acquired for free by any INSERT referencing the row and masks a deleted
  `FOR UPDATE` from every naively-worded counterparty; the criterion names a
  mode the mutation changes the answer for (`FOR NO KEY UPDATE` for the task
  lock) and the plan states which competing lock would otherwise mask it
  (phase 7 shipped TWO named mutations inert against the obvious observable).
- **P-Q fourth extension (phase-7 review L2):** a prescribed FIXTURE is
  checked against the engine semantics it will meet, exactly as a named
  mutation is checked against the implementation it will meet — the C2
  direct-INSERT fixture's own FK lock made its target identity unraisable
  (the intruder blocks at step 1, then is superseded normally: two rows, one
  current, no error).
- **P-R second extension (phase-7 review L3):** a criterion demanding a
  SECOND-SESSION observation names its committing harness in the plan
  (`database._session_factory()` + committing teardown, phase-4 recipe) —
  the default `db_session` fixture never commits, so the criterion was
  unsatisfiable as written and shipped satisfied by the very identity-map
  comparison it forbids.
- **P-AB (phase-7 review L4, parameter-gated helpers):** when a shared helper
  performs side effects gated by a mode/kind parameter, the plan enumerates
  **which effects the parameter gates, one line per effect** — anything
  unlisted is a finding. B1 happened because `kind` gated the chain S1, the
  history record, the audit row and the event, but not the mirror; the helper
  was one `if` from correct and no list existed to notice.
- **Deferral rule (phase-7 review L5):** deferring a named mutation to the
  reviewer defers WORK, never RISK — the deferring round's review prompt
  requires the deferred criterion itself be RE-DERIVED before it is run
  (three of five deferred rows needed their criterion rewritten before they
  could bite; the deferral had hidden plan defects, not just unrun tests).
- **P-L second extension (phase-7 review L6):** the tracker's own figures are
  evidence and are verified at consumption like any hash — row 7 carried
  suite numbers matching neither the handoff nor reality, and the
  coordinator's consumption verified file hashes while transcribing the
  figures unchecked.
- **P-AB companion clause (phase-7 re-review-r2 L1):** an effect enumeration
  is written by READING the parameter's occurrences, never the author's model
  of them — the check is one grep of the parameter name in the file, every
  hit either an enumerated effect or explicitly excused. P-AB failed its own
  first application: the cycle that created the rule shipped an enumeration
  listing an effect (`the audit row`) the parameter does not gate, and the
  error pointed the dangerous way (a reader would conclude projections write
  no audit row). The converse of P-AB binds: listing an ungated effect is the
  same defect wearing the other sign.
- **Adoption-fidelity rule (r2 L2):** when a fix cycle adopts artifacts
  authored by a prior session, the originals are preserved WITH HASHES at a
  named path and the adoption is verified by DIFF, not by claim — this is
  what let a ~50-row coverage blocker collapse into one cycle safely
  (`probes/reviewer_r1/` is the template).
- **P-I ninth extension (r2 L3):** a named mutation is phrased so two
  independent agents produce the SAME mutant text ("delete
  `.with_for_update()` at the definition site in `_load_task_and_primary`"),
  and the reviewer reports match/mismatch against the declared mutant hash —
  five of seven phase-7 fix mutants reproduced byte-for-byte, the first
  re-derivable (not merely re-confirmable) ledger in the project.
- **P-S template (r2 L4):** deleting an unbuildable criterion direction is a
  legitimate fix — when P-S establishes a path unreachable, the correction is
  the recorded note plus the one arbiter that CAN exist (phase 7: the
  unit-level `INDEX_IDENTITIES` translation row), never a fixture invented
  around the engine's semantics.
- **Deferral cap (phase-8 review L1 — sharpens the deferral rule):** a cycle
  may defer at most a HANDFUL of rows, and **never the rows guarding the
  phase's own named mechanism** — a per-row deferral is compliant in form and
  worthless in substance when the whole ledger is deferred (phase 8 reached
  review with a 2-of-18 mutation score on its central emissions).
- **Expected-red rule (phase-8 review L2):** a named mutation names its
  ARBITER too — mutation site → **expected red pytest node id**. Where the
  plan named a test (A13, A6) the mutation bit; where it named only a site,
  the implementer wrote a test with the right name and no power.
- **Call-graph rule (phase-8 review L3 — the enforceable P-V form):** no two
  rows of an enumerated criterion may share a call graph — twelve serializer
  echoes satisfied "one id per member" while exercising nothing; producers
  are enumerated per row.
- **Endpoint-boundary rule (phase-8 review L4):** a money-boundary criterion
  names the ENDPOINT's response, never only the serializer — the phase-1
  lesson did not travel because C9 was phrased about a payload, and serving
  WORKER the full manager payload survived every shipped row.
- **Hand-written-literal rule (phase-8 review L5):** a completeness table is
  a hand-written literal, never derived from the surface it audits — a table
  computed from `router.routes` cannot disagree with the router by
  construction.
- **Red-coverage flag (phase-8 review L6):** when a phase's touch point's
  only existing integration coverage is in the ESTABLISHED FAILING set, the
  plan says so and a new green row is mandatory, not optional (the reopen
  hook landed on baseline failure #5).
- **Row-list tick rule (phase-8 re-review-r2 L1):** a fix instruction saying
  "every row" carries the ROW LIST and the fixer ticks it — the two C7
  members left as echoes were precisely the two A1 called out by name; the
  rows hardest to build are exactly the ones that stay decorative.
- **Pipeline-ends rule (r2 L2):** a criterion about a data PIPELINE needs a
  row at each end — C2 proved the rollup columns separate and stopped one
  step upstream of the clause that matters for money (§8A.1 reads only the
  working column); enumerate criteria along the data path, not by mechanism
  name.
- **P-J fifth extension (r2 L3, stability rows):** non-vacuity applies to
  "nothing changed" rows — a criterion asserting STABILITY must prove the
  perturbation would otherwise have been visible (the C5 supersession row
  deleted a row nothing in the call graph reads).
- **Set-assertion rule (r2 L4, earned by the reviewer against their own
  probe):** a discriminating fixture never rests on physical row order —
  assert over the candidate SET, not an unordered `scalar()`'s pick; a
  flaky row is corrosive in a project gating on byte-identical failure
  sets.
- **P-I ninth extension restated (r2 L5, HARD ledger field):** the mutation
  ledger carries the per-row MUTANT hash as a required column — restoration
  hashes alone make a ledger re-confirmable but not re-derivable.
- **Structural-filter rule (phase-8 re-review-r3 L1):** a filter-deletion
  mutation whose CORRECT row remains in the candidate set cannot be
  arbitrated order-independently by a behavioural assertion alone — the
  mutated query may pick the right row by accident. Such criteria carry a
  STRUCTURAL arbiter over the compiled statement (the literal clauses
  present in the SELECT) with the behavioural row as companion. (Earned by
  the reviewer against their own r2 repair spec, which traded a
  false-positive flake for a possible false-negative one.)
- **Distinct-values rule (r3 L2):** exclusion criteria ("these columns are
  never read") use DISTINCT nonzero values per excluded column — 30/40/50
  makes each leak produce a unique wrong total naming its culprit; equal
  values only say "something leaked".
- **Scenario-fixture rule (r3 L3):** a row's fixture must reach the
  SCENARIO the criterion names, not only its mechanism — C5's "supersession
  after close" row never closed the task, so two compared columns were
  vacuous and the row's name overstates its coverage for every later
  reader.
- **Repair-adversarial rule (r3 L4):** a reviewer-specified repair gets its
  own adversarial pass in the NEXT round — a specification naming its own
  acceptance condition gets exactly that and no more, so the condition must
  be the property actually wanted, stated structurally where possible.
- **Per-alternative rule (phase-8B review L1):** a criterion enumerating
  alternatives ("deleted OR superseded-only") carries one row per
  alternative, and each row's expected-red mutation is the CONJUNCT that
  alternative uniquely exercises — a fixture can satisfy the outcome for a
  reason that is not the reason the missing row would test, and the shipped
  row then looks like coverage of both.
- **Rule 11½ second failure mode (8B L2 — capture-locals):** cleanup that
  depends on a transaction the failure destroyed cannot run — a `finally`
  dereferencing ORM instances the rollback expired raises before deleting
  anything. Teardown identifiers are captured into PLAIN LOCALS before the
  `try` (phase 8B holds the defect and the correct pattern side by side:
  C4 rows 2/3 vs row 1) **Companion check (r2, validated): run the RED
  path once under a mutant, then query state — zero residue is the
  evidence; the cheap generic verification for any committing test.**
- **P-I tenth extension (8B L3 — pin the boundary):** a deletion-shaped
  named mutation states its LINE RANGE, and "observed red" states the
  SCOPE it was observed over — four of five 8B mutants reproduced
  byte-for-byte from prose alone; the fifth failed only on an unpinned
  deletion boundary, and one ledger row under-reported a 9-node blast
  radius as "1 failure".
- **Fence-contradiction rule (phase-9 review L1):** before routing a
  rewrite-in-place over a document, grep the target for the terms the
  rewrite invalidates — widen the fence to the contradiction set or record
  "partial rewrite accepted, residue filed" as the INTENDED end state; a
  line-range fence over a document whose neighbouring sections assert the
  opposite produces an artifact that argues with itself (the implementer's
  fence discipline was correct; the fence was not).
- **Mirror-column rule (phase-9 review L2):** a criterion repairing a
  mirrored table names WHICH COLUMNS are in scope, and a mirror carrying a
  contract gets at least a grep-level arbiter — the backend's copy was
  guarded and stayed right; the unguarded frontend mirror shipped
  Required=Yes beside "always rejected".
- **Direction-declared rule (phase-9 review L3):** a one-directional
  arbiter says so in its own docstring — "nothing invented" is not
  "nothing missing", and the unguarded direction is where S2's terms:[]
  gap slipped through.
- **Contract-vs-tree rule (phase-9 review L4):** a newly authored contract
  file's acceptance includes ONE GREP PER NORMATIVE RULE confirming the
  rule describes the tree or states what it supersedes — P20's one
  unqualified sentence contradicted ~20 modules of correct practice.
- **Birth-anchor rule (8B L4):** a graph node born mid-project inherits
  UNVERIFIED anchors for everything it already did — the spans describing
  the phase's NEW behaviour get implementer scrutiny, the pre-existing
  behaviour's spans do not; the post-approval pass reads them all
  (8B: the two new edges exact, the two pre-existing ones imprecise).
...
- **Current head (updated at phase-8 closeout, 2026-08-15):** `c1d2e3f4a5b6`
  — phase 8's one migration (enum label) after `be9dfe42a035`:
  `c1d2e3f4a5b6_add_process_item_cost_result_task_type.py`.
  **Suite baseline at phase-9 closeout — v1 CLOSURE (2026-08-15): 2249 / 23
  (byte-identical phase-1 list, reviewer-diffed) / 1 deselected = 2272
  selected (2273 collected).** Prior: phase-8B 2184/23/1 = 2207; Prior baselines for history: phase-8 closeout 2138/23/1 =
  2161 selected; phase-7 closeout 2076 / 23 / 1 = 2099 selected (reviewer-measured foreground, +39
...
- (2026-08-14, phase-6 r1 N3) `Base.metadata.create_all` is broken repo-wide (pre-existing, vindicates D9's harness replacement) — fix only if a phase ever genuinely needs metadata-create.
- (2026-08-15, phase-9 projection F14) the committed-current three-clause
  filter has FOUR production sites beyond r3-N1's three status-query sites
  (`process_item_cost_result.py:66-68` — the worst silent-failure candidate,
  it resolves what gets PERSISTED; `commit_item_cost_evaluation.py:277-280`,
  `:288-290`; `create_item_cost_projection.py:32-34`) — extend the
  structural compiled-statement arbiter to them only-if-cheap; the boundary
  at three is a RECORDED decision (r3-N1's own wording), not an oversight.
