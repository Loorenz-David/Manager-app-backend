...
| 5 | Price-scenario: injected clock, shared reconciliation, §6B | **`CHANGES_REQUESTED`** | 2026-08-24 | Codex (fix r3) → coordinator consumption | **Fix round 3 consumed 2026-08-24 → fix round 4 dispatched (two items, no production change).** **Both blocking findings closed, B1 in the preferred form:** C8 now drives `get_task_price_scenario` **end to end** and asserts the **served payload** (which also resolves N3's wire-reach half), and the mutation that left the entire repository green one round ago now reddens it on `assert 375 == 600` — verified by the coordinator at source. **B2 closed with every prescribed element declared one by one**: DB-backed, real session, the prescribed `FakeDatetime` returning `ctx.now - 1s` / `ctx.now + 1s`, and `frozen = 2026-10-30` chosen so the fixture's existing `max(closed_at) = 2026-08-01` lands exactly on the 90-day boundary — **the fixture was not modified**, which was the cheaper and better move. Perimeter one test file, production untouched; summands right at 15 + 2 = 17; stamp **2707 / 21 / 1**, 21 IDs enumerated, ∅/∅. **★ One should-fix, measured by the coordinator, and it is round 3's own doing.** Pointing C1(b) at `narrowed_task` made the clock row sensitive to narrowing: `specs = ()` reddens **four** tests and `None` at the call site reddens **two**, both including `test_c1b`, against declared bites of one row each. **The extra reds are correct behaviour and nothing is unarmed** — but the declared bite sets are narrower than the truth, and **M7's only composed guard now reddens for M1 reasons, so its red no longer names the clock**. That is **§8A lesson 3 biting one round after it was written**. → §6A C1(b) amended to run on `plain_task`, which decouples the row and narrows both bite sets in one edit. Plus: C1(b)'s numeric assertion precedes its byte-identity assertion, so under C1(i) **byte-identity never executes** — reorder. **Notes closed correctly:** N1 and N4 fixed in test; N2/N3/N5 declared rather than covered; **S1 correctly left alone** — no test was invented for a mechanism with no observable. Fix round 4: `prompts/implementer/20260824_plan5_fix_round4_prompt.md`. |
