...
| 8B | Inline item prices at task creation | `plans/phase_8b_inline_task_prices.md` | ⚑ (R18-1 — owner scope addition 2026-08-15) | PROMPT_READY | 2026-08-15 | coordinator (round 18) | §7B.6: the task-creation item block accepts the valuation trio (expected_sale_price_minor / purchase_cost_minor / currency, mirroring ItemValuationRequest); newly created item → valuation v1 via the registered chain writer BEFORE the auto-commit savepoint (priced in one call, R13-1); matched existing item → REFUSE (conservative default, projection may card); legacy keys stay 422 ITEM_MONEY_MOVED. Ships BEFORE phase 9 (the frontend handoff documents the post-8B flow). Projection r0 prompt authored (`prompts/reviewer/2026-08-15_phase8b_projection_r0.md`; axes: find-or-create seam, existing-item card pressure-test, multi-item/quantity enumeration, savepoint-interaction rows, validation boundary, expected-red discipline). **Projection r0: AMENDMENTS_REQUIRED** (18 rows: 5 H / 6 M / 7 L; 1 owner card; live-verified incl. collection 2162, graph 173/256 rev 45b72196, no create_task node exists). Standouts: the 'exact mirror' gloss FALSE against shipped code (PUT currency unconditionally required); C1's 'auto-commit fires' false for 3 of 5 trio shapes; C4 needs the maybe_begin OWNER harness + the item-byte-unchanged rollback row; the mixed-payload row was a definition-order coin flip; currency-alone DECIDED accepted-and-ignored (DB CHECK makes the row impossible; 422 reproduces the recorded §10A.3 hazard); L9 verified the savepoint interaction by construction. Coordinator routed all 18 (2026-08-15): owner card 1 → **R18-3 branch B** (refuse IFF a current valuation exists — INV-V1 predicate; never-valued → v1; deleted/superseded-only → NEXT version, explicit act distinct from R15-1); §7B.6 lettered (a) trio shape + (b) branch B; §4.7A writers += create_task; §11A.5(c) corrected; §6.4 += ITEM_COST_INLINE_PRICE_ON_PRICED_ITEM (L11's name adapted for B); plan GOVERNING block B1–B10; L14/L15/L18 → phase-9 R18-2 sentences. State → PROMPT_READY; implementer prompt authored (`prompts/implementer/2026-08-15_phase8b_implement_r1.md`) |
