...
|---|---|---|---|---|---|
| 3 | D9 + OD-10's boundary | **PROMPT_READY** (fix r2) | 2026-08-21 | Opus 5 (review r1) + coordinator (fold) | **Review r1 folded. 0 blocking, 2 should-fix, 6 notes — the production code is correct and no production line is in scope for the fix.** Third consecutive phase whose every finding is in a plan, a criterion or a document. Reviewer perimeter verified; **both probe reverts proven by SHA-256 against its declared pre-probe digests, matching byte-for-byte**; it cited the implement stamp instead of reproducing it and spent the budget on two mutant shapes no ledger had run — the evidence policy working exactly as intended. **S1 is the coordinator's defect, in three documents:** intention §5.3A, plan §5 C6 and this tracker all claimed C6 **row (b)** stops a future "null it whenever `status == infeasible`" edit. Measured false, and confirmed by the coordinator at source (`values[10].allowed_worker_minutes = Decimal("0.00")`): C6b's own payload is `infeasible`, so status-blanking returns exactly the `null` it asserts — **the row passes under the edit it was documented as stopping**; the guard is **row (a)**. Attribution swapped in all three, and C6b's fixture re-specification made **required** — its `null` has two independent sufficient causes and its comment claims an independence the fixture cannot show. **S2: the frozen percent's over-budget region is guarded by nothing in the repository** — clamping it at `100.00` leaves the whole suite green (∅/∅ at L4), because every fixture pinning a numeric frozen percent sits at ≤ `100.00`. **Sharpened at consumption beyond the finding:** the region is not merely untested but *computed on every suite run and observed by nothing* — `test_phase8_serializers.py:_result()` (`actual 2.00 / variance −1.00`) makes `test_budget_status_worker_surface_excludes_money` serve `result.percent_consumed == "200.00"` while asserting only `allowed_worker_minutes` and four key absences. Added as **C6c** (frozen `"150.00"`, both faces). **Eleventh instance of the row-that-cannot-fail class and a fifth distinct shape — degenerate output range**: §5A's rule was applied to the numerator and never to the output's regions; §5B now binds criteria to enumerate the regions their authority names. Fix prompt `prompts/implementer/2026-08-21_phase3_fix_r2.md` — test-only perimeter, correction clauses verbatim, N2/N3/N5/N6 record-only, and a **new required ledger field**: state which row did *not* bite, not only which did. |
...
| 4 | Closeout handoff (six §7 obligations, headline: retire the frontend's interim flag) + the five-node graph delta | **BLOCKED** (gate: test-environment work) | 2026-08-21 | — | `plans/plan_4.md`. **⛔ Does not start until `pytest-xdist` + per-worker DB isolation ships and the baseline failure-ID set is re-enumerated under the new runner** — owner decision 2026-08-21, conditions in §6's gate block. Its closeout publishes the baseline `narrow_typical_work_times` D23 consumes, so that baseline must be the new runner's, stated with its runner. Projection **WAIVED**: documentation only, no mechanism — waiver recorded here per charter. Full review round regardless. **+C8** (2026-08-21): the OD-10 correction to the published frontend handoff, as a new dated document. |
