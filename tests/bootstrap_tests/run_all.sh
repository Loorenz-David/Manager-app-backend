...
start_task_router_if_needed() {
  ...
  if pgrep -f "beyo_manager.workers.task_router_process" >/dev/null 2>&1 || pgrep -f "beyo_manager/workers/task_router_process.py" >/dev/null 2>&1; then
    echo "✅  Task router already running"
    return 0
  fi

  echo "→ Starting task-router worker for sleep-mode coverage..."
  APP_ENV=development "$VENV_PY" -m beyo_manager.workers.task_router_process >/tmp/bootstrap_test_task_router.log 2>&1 &
  TASK_ROUTER_PID=$!
...
start_delayed_scheduler_if_needed() {
  ...
  if pgrep -f "beyo_manager.workers.delayed_scheduler_runner" >/dev/null 2>&1 || pgrep -f "beyo_manager/workers/delayed_scheduler_runner.py" >/dev/null 2>&1; then
    echo "✅  Delayed scheduler already running"
    return 0
  fi

  echo "→ Starting delayed scheduler..."
  APP_ENV=development "$VENV_PY" -m beyo_manager.workers.delayed_scheduler_runner >/tmp/bootstrap_test_delayed_scheduler.log 2>&1 &
  DELAYED_SCHED_PID=$!
...
start_recurring_scheduler_if_needed() {
  ...
  if pgrep -f "beyo_manager.workers.recurring_scheduler_runner" >/dev/null 2>&1 || pgrep -f "beyo_manager/workers/recurring_scheduler_runner.py" >/dev/null 2>&1; then
    echo "✅  Recurring scheduler already running"
    return 0
  fi

  echo "→ Starting recurring scheduler..."
  APP_ENV=development "$VENV_PY" -m beyo_manager.workers.recurring_scheduler_runner >/tmp/bootstrap_test_recurring_scheduler.log 2>&1 &
  RECURRING_SCHED_PID=$!
