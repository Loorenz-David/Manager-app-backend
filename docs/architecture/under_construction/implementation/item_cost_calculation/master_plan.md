...
| 4 | Configuration services | `plans/phase_4_configuration_services.md` | ⚑ (rows 15–20) | **IMPLEMENTED** | 2026-08-12 | coordinator | groups, chains, guarded deletes, config status; projection r0 prompt authored (`prompts/reviewer/2026-08-12_phase4_projection_r0.md`; forwards: N3 enum order, N7 persisted-rate arbiter, S4 request parse). Projection r0 AMENDMENTS_REQUIRED (24 rows, 6 blocking; 1 owner card) — fully routed: R11-1 canonicalize-then-derive (intention round 11; owner: round, never refuse); B2 index-discrimination idiom; B3+S7 registry += 3 dual-path identities + audit vocabulary; B4 concurrency harness block; B5 split C6 mutations; B6 structural enum-order guard + N7 in-phase row; S1-S9 + N-pins folded into tasks/criteria C1-C11; N7-consumption -> phase 5, C6 counterparty -> phase 7; implementer prompt authored; Codex implementation: 72 focused tests passed, full suite 1755 passed / 23 established failures / 1 deselected with the failure set unchanged; one batched architecture-graph delta recorded; checkpoint and handoff follow. |
| 4B | Category-driven group selection (§7C, round 12) | `plans/phase_4b_category_selection.md` | ⚑ (selection = S1 mechanism; inventory row 19 superseded by §7C) | NOT_STARTED | 2026-08-12 | coordinator | owner scope decision pre-v1: major_category NOT NULL + INV-G3 + classifier rework + per-category status; plan to be authored by a focused planner session (`prompts/planner/2026-08-12_phase4b_planner_r1.md`); starts only after phase 4 APPROVED |
...
| `production_cost_groups.major_category` (round 12, §7C) | reuse the items-domain major-category enum | **reuse** its PG type with `create_type=False` (ownership stays on the items-domain column; exact Python class + PG type name pinned by the phase-4B planner after verification in-tree — R2-1 rule) | wood \| seat |
| economics status | `EconomicsStatusEnum` | **none — never persisted** | code-owned (§11A.4, catalog lesson); the 11 members of §11A.4, lowercase values. **Declaration order carries NO precedence** (corrected 2026-08-12, phase-4 projection B6: as shipped the declaration order differs from §11A.4's evaluation order — precedence lives in `configuration.py`'s explicit ordered sequence, never in enum iteration) |
...
- **Audit event vocabulary (registry-authored 2026-08-12, phase-4 projection S7)** —
  ...
  (phases 5/7 add `item_valuation.*` / `item_cost_evaluation.*` rows here before
  use — never free-formed in a command.)
...
- **Domain (pure, no I/O):** `app/beyo_manager/domain/item_economics/`
  ...
  **Registry decision (review N2, 2026-08-12; count corrected per re-review N8 —
  ...
  projection S3**; precedence from an explicit ordered sequence, never enum
