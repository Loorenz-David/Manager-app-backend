...
|---|---|---|---|---|---|
| 4 | Closeout handoff (all seven §7 obligations) + the five-node graph delta | **APPROVED — PHASE 4 AND THE PIPELINE CLOSED** | 2026-08-22 | Codex (r1, fix r2, fix r4) + Opus 5 (review r3, re-review r5) + owner (OD-11) + coordinator | **Five rounds, 0 blocking outstanding, no code, and the pipeline's shipped promise is delivered.** Checkpoints `80b8cca` → `3df02ae` → `4e79e9d` → `8bc8984`; approval-gate commit follows. **`git diff 0aae85e HEAD -- app/` is empty across the entire phase** — phase 4 changed nothing under `app/`, which is why the published baseline is `dc76db8`'s tree and why every round cited the gate stamp instead of re-measuring it: **0 L4 runs in the whole phase**, five rounds long. **Findings by author, and the pattern held for a fourth consecutive phase: every blocking-or-should-fix finding was in a plan, a criterion, a coordinator artifact or a prose sentence — none in a mechanism.** Pre-implementation, the coordinator's own reconciliation caught two blocking plan defects (**F1** §6A still instructed the closeout to publish phase 2's serial-runner / dev-database number — the exact baseline this phase was gated four weeks to avoid handing D23; **F2** closeout obligation 7, the phase's whole purpose, had **no acceptance criterion** — C9 added). Implement r1 shipped a graph delta of **edges only**, leaving the four projection descriptions asserting the settled-only basis (**B1**) — declared under rule 14, but the stated reason (*"preserved rather than rewritten through maintenance"*) did not survive a dry-run `preview_maintenance_changes`, which accepted the edit on a `human_confirmed` node with no cascade and no adjudication. **The ledger had hidden it by describing the frontend document's prose in the graph obligation's Result column** — one row spanning two artifacts reports on whichever was easier to satisfy, now a §5 rule. **S1 was a finding against this coordinator's own criterion:** C5 paraphrased intention §5.4's *"record deletion … is not named to the client"* into *"not named as a cause"*, and the document satisfied the paraphrase while violating the authority. Every actor was correct against the artifact in front of them, and for two of them that artifact was a compression written here. **It was caught only because the coordinator judged the literal met, declined to decide it, and reserved it for the reviewer** — the argument for reserving judgment calls rather than settling them in a prompt. C5 now quotes the clause; the general rule is in §5. **S2:** the published baseline omitted its instability caveat — measured consequence, since neither flaky test is a *member* of the 21, a flake reaches D23 as a **new** failing ID, i.e. as a regression they caused. §7's schema gained a required field and §6's gate block now states what it does **not** supersede, which is how the caveat went missing. **Owner card answered same-session → OD-11** (*"we keep them ( recommended option )"*): the graph queue is **adjudicated and closed — 5 promoted, 0 pending, 0 stale, 0 diagnostics**, revision `7241b831…`. **Re-review r5 APPROVED with 0 blocking / 0 should-fix / 3 notes**, spending **0 L4 and 0 L1** and buying nine facts no prior round held — the fix's dirty-tree digest reproduced cryptographically; the 21⊂26 containment and the five-removed enumeration derived independently against §6 (both directions empty, a **second** derivation of C9's subset claim); and **obligation 2 proven from git history rather than from the document's own account of itself** — `git diff 80b8cca HEAD --name-only -- docs/handoff/to_frontend/` returns **exactly one file**, the new dated document. Its three notes (N4 direction of the third intermittent, N5 basenames-vs-node-IDs, N6 a non-root-relative path) were **folded at closeout, not deferred**: §6 now carries both full node IDs and §7's schema carries the direction clause. **Intention folded at round 4i** ("snap" governs the smoothing baseline, never the rendered value — a *word*, not a mechanism, was the defect, the second time in this pipeline). **Five rules earned into §5**; the published approval baseline is in §7's table and, authoritatively for a successor, in the frontend handoff's §7. 11 session artifacts archived to `archive/plan_4/`. **What ships: the go-live statement that retires the frontend's interim verdict-suppression flag** — the single binding promise this pipeline owed, made 2026-08-19 and kept 2026-08-22. |
| 4 | Closeout handoff + the five-node graph delta | **APPROVED** (re-review r5) | 2026-08-22 | Opus 5 (re-review r5, independent) | **0 blocking, 0 should-fix, 3 notes — the phase's last gate closes, and with it the pipeline.** Handoff `handoffs/reviewer/2026-08-22_phase4_rereview_r5_handoff.md`. Tree `e6a49c6`, porcelain clean. **Perimeter reconstructed, not accepted:** five documents across the three commits in range, each inside a declared perimeter; `-- app/ .archgraph/` **empty**; no external-stream attribution needed. **The declared dirty-tree digest reproduces** — `git diff --binary c543640 4e79e9d \| shasum -a 256` = `db0045f66f63d5ab…` exactly, so C7's before-and-after guard measured the delivered content. **P2 — mode 2 reads correctly after the deletion, re-derived event by event against §6A A:** the surviving "**any record**" is load-bearing, because §6A A flags **E2** (mark *any closed* record) as *"not named in §6"* — a fix that narrowed that clause while deleting its neighbour would have silenced E2 with no criterion catching it. E4 covered; **E3 correctly absent** (§6A A records it as a *rise*, not a decrease); **E6 correctly absent** (a 404, a different contract); E5 absent, which is S1. The surviving rule is cause-independent **by construction**, so r3's justification for the deletion holds. **S1 closed as a class with a different instrument than the coordinator's** — 11 alternative terms (`eras\|purg\|remov\|destro\|discard\|wipe\|reset\|hard delete\|soft.delet\|not a cause\|not a shipped\|capabilit`) plus every `record` occurrence → six hits, all benign, **zero sites naming record deletion in any register**. **P3 — the instability caveat is actionable, not merely a warning:** "repeat and ID-diff before concluding that the set has changed" is **direction-neutral** and discriminates correctly on an extra named ID, an extra unnamed ID **and a missing ID**, without needing the third test's identity — which is what makes publishing an unrecoverable identity useful rather than decorative. Both named IDs **exist at source** (`test_phase4_fix_coverage.py:240`, `test_process_shopify_products_integration.py:82`) and non-membership re-verified **by basename** — stronger than the earlier full-path grep, since it also catches a same-named test at another path. 0 and 0. **P4 — both "exactly" claims survive:** three is still right and the right three, derived from §6A C rather than the document (its five bullets are three modes plus the general "any decrease" rule and the standing `share_state` rule, both correctly carried **outside** the numbered list); E1–E6 collapses onto exactly these three. Re-derived because obligation 5 records that this row *"originally said 'exactly two ways'"*. **Nine new facts, none held by a prior round:** the digest; **the published 21 are all members of §6's 26** (`comm`, ∅ foreign); **the five-removed enumeration reproduces by independent arithmetic** — `master26 − doc21` equals the published five **exactly, both directions**, a second derivation path to C9's fold-time result; **obligation 2 proven structurally** — `git diff 80b8cca HEAD --name-only -- docs/handoff/to_frontend/` is **exactly one file**, the new document, so "no published handoff was edited" comes from git history rather than the document's claim about itself, which is the strongest available form of that check; `dc76db8`'s subject matches character-for-character and `git diff dc76db8 HEAD -- app/` is empty, so the tree-identity sentence D23 leans on is true *as of this review*; `pytest.ini:2` = `-n 6 --dist loadfile`; **all twelve provenance-appendix paths resolve**; C9's seven elements all present; 21 + 2576 = 2597. **Notes, all improvements to a correct document.** **N4** — §7 names the third intermittent test but drops §7's attached consequence *"(so the set can shrink too)"*, and the preceding non-membership clause implies an additive-only reading; **kept a note, not a should-fix** — nothing false is stated, the schema's three required elements are all carried, and the published rule is direction-neutral so a reader following it is protected on a shrink. **N5** — the two named intermittents ship as bare basenames while every other ID in §7 is a full node ID, in a section whose own instruction is a mechanical ID-diff (full node IDs measured verbatim in the handoff). **N6** — the appendix's `planning/intention.md` is not repo-root-relative, unlike its twelve siblings, and collides with the successor project's own file of that name. All three dispositioned to §6/§7 at closeout; none is owed by phase 4. **Evidence: 0 L4, 0 L1 — nothing run.** Both gate stamps cited by tree identity; the docs guard **deliberately not re-run** (fix r4's 59 passed covers content proven byte-identical to `4e79e9d`, the commits since touch nothing the guard reads, and its non-vacuity is already measured by the planted-token probe) — and r3's recorded lesson about justifying a re-run by a *containing* directory was not repeated. **No mutation probe applied**; every instrument was a read, a hash, or set arithmetic over text — nothing to revert. **Closeout readiness: all seven of §7's obligations discharged**, checked one by one; the baseline `narrow_typical_work_times` D23 consumes is ready to publish as it stands. |
| 4 | Closeout handoff + the five-node graph delta | **REVIEWING** (re-review r5) | 2026-08-22 | Codex (fix r4) + coordinator (consumption) | **Fix r4 consumed — all four corrections in, rule 14 correctly declared nothing omitted. Re-review r5 dispatched: delta-scoped, the phase's last gate.** Checkpoints `4e79e9d` (+ `8bc8984`). Perimeter exactly three files; `git diff c543640 HEAD --name-only -- app/ .archgraph/` **empty**, and `archgraph_status` still reads `7241b831…` / 194 / 296 / 0 pending / 0 stale / 0 diagnostics — byte-identical to the post-adjudication reading, so "tool-recorded state: none" is **proven, not accepted**. **The declared dirty-tree digest reproduces cryptographically:** `db0045f66f63d5…` equals `git diff --binary c543640 4e79e9d` over its three tracked files, so the content the L1 run measured is byte-identical to what shipped. Last round's row named a SHA and described its dirt in prose; that note went into this round's prompt and was answered — **a note that changes the next round's artifact is worth more than a finding that scores the last one.** **Coordinator class sweep beyond the finding:** S1 named one sentence, and this pipeline's signature failure is a correction applied at one site while its class survives elsewhere. `grep -niE "deletion\|deleted\|delete"` over the whole document returns four hits, all benign ("non-deleted steps" in the cost sentence, the quoted commit subject in the tree-identity bullet, two test IDs inside the published 21) — **no other site names record deletion as a client-visible event.** Closed as a class. The fix **correctly declined** to claim the isolation work retired the flakiness, which the clause permitted only if measured. **Evidence: 0 L4, 0 L1** — nothing run; the fix's own stamp covers content proven identical to the checkpoint and no file the docs guard reads has moved since, so a re-run would be over-evidence. Verification went to the digest and the sweep, both producing facts no prior round held. **Why re-review rather than approval:** r3's verdict was `CHANGES_REQUESTED`, and only a reviewer moves a phase to `APPROVED` — a coordinator verifying its own fold is the loop the gate exists to break. Prompt `prompts/reviewer/2026-08-22_phase4_rereview_r5.md`. |
| 4 | *(prior row — fix r4 dispatched)* | *superseded (IMPLEMENTED, fix r4)* | 2026-08-22 | Codex (fix r4) | **Fix r4 closed S1, S2, N1 and N2 in the declared docs perimeter; no graph or `app/` changes.** §5 mode 2 no longer names record deletion; the closing rule names the smoothing baseline; §7 publishes the two named intermittent tests, the unrecoverable third intermittent test, the repeat-and-ID-diff rule, and Redis's 23-failed / 2-errors diagnostic. Docs guard: 59 passed. Handoff: `handoffs/implementer/2026-08-22_phase4_fix_r4_handoff.md`. Checkpoint follows.** |
...
| 4 | Closeout handoff + the five-node graph delta | **CHANGES_REQUESTED** (review r3 consumed) | 2026-08-22 | Opus 5 (review r3) + coordinator (fold) | **First external review: 0 blocking, 2 should-fix, 3 notes, 1 owner card — answered the same session. Fix r4 dispatched; the graph obligation is now fully discharged.** **Every load-bearing measurement re-verified before acting:** neither named flaky test appears in the published 21 (0 and 0 — S2's whole basis); `pytest.ini` carries `-n 6 --dist loadfile` verbatim; `get_task_price_scenario.py` touches `budget_status.status` and `.item_binding` and nothing else (239/242/298–299) and imports neither `StepStateRecord` nor `live_worked_seconds`. Reviewer perimeter exactly its three declared files. **S1 is a finding against this coordinator's own criterion.** C5 read *"record deletion NOT named as a cause"*; intention §5.4 and §7 obligation 5 both read *"is not named to the client"*. The document named it **as a non-cause** — satisfying the paraphrase, violating the authority. Every actor was correct against the artifact in front of them, and for two of them that artifact was a compression written here. **The coordinator judged C5's literal met, declined to decide it, and reserved it for the reviewer — that routing is the only reason it was caught**, and it is the argument for reserving judgment calls instead of settling them in a prompt. C5 amended to quote the clause; the general rule folded to §5. **S2** — the published baseline omitted its instability caveat, and the consequence is measured: since neither flaky test is a **member** of the 21, a flake reaches D23 as a **new** failing ID, i.e. as a regression they caused — the exact misreading the subset bullet prevents one bullet earlier. **§7's baseline schema gains a required field (known instability)** and **§6's gate block now states what it does NOT supersede**, which is how the caveat went missing. **N1** folded into the same fix. **N2 was decided the other way and is not a defect:** "snap" carried two referents across §5.4, §6A C and §3.3 and the implementer carried §5.4 faithfully — resolved **upstream** at intention **round 4i** ("snap" governs the smoothing baseline, never the rendered value), with the reviewer's endorsed one-noun clarification carried into the document. **Owner card ANSWERED (owner, 2026-08-22, verbatim: *"about the owner card, we keep them ( recommended option )"*) → OD-11.** The graph queue is **adjudicated and closed: 5 promoted, 0 pending, 0 stale, 0 diagnostics**, revision `7241b831…`, record `.archgraph/reviews/2026-08-22T13-26-50-282Z--7ddf0c.yml`. Four items were the card's subject; the fifth was uncontested and source-verified by P2, promoted under the same adjudication with that scope stated in OD-11 so it is correctable in one line. **Review r3 verified all five node descriptions TRUE at source** — a check no criterion required, which is now lesson 5 in §5. **Coordinator note, recorded not raised:** E1 justified a re-run by tree difference "in `docs/`" while that guard's roots are `docs/handoff/` and `docs/domains/item_economics/`, neither touched — tree-matched, three seconds, no round; the pattern is named because justifying a re-run by a directory that merely *contains* the read surface is how a two-minute L4 gets justified next time. Fix prompt `prompts/implementer/2026-08-22_phase4_fix_r4.md` — four edits, one file, no graph work. |
| 4 | *(prior row — review r3 dispatched)* | *superseded (CHANGES_REQUESTED, review r3)* | 2026-08-22 | independent reviewer (Claude Opus 5) | **First external review, full checklist against C1–C9 and the semantic authorities. 0 blocking, 2 should-fix, 3 notes.** Perimeter verified first: `git show --stat 3df02ae` is **exactly the allowed 10 files**; the two extras in `git diff 80b8cca 3df02ae` belong to the coordinator commit `e13923f` between the checkpoints, so no violation and no external-stream attribution needed. **The document is substantially correct** — every criterion discharged, every numeric claim traced to its deriving section, and the two most falsifiable graph claims confirmed **exactly** at source. **S1 (should-fix) — record deletion is named to the client**: intention **§5.4** and master **§7 obligation 5** both say it *"is not named to the client"*, not merely "not named as a cause"; C5's narrower literal IS met, so the coordinator's reading was right and the document violates the authorities C5 compressed. Not blocking because no behavioural harm follows — measured at source that `reset/reset_app.py` deletes task steps, tasks and the workspace alongside `delete_step_state_records.py`, so no surface survives to show the decrease; and **no published frontend handoff has ever named record deletion as a cause** (grep over all 18 documents), so the sentence retires nothing. It is also the one sentence in §5 describing our write surface rather than their behaviour — the register §6A C excludes, and the same test that correctly kept §6A B's two-drop fact out. **S2 (should-fix) — the published baseline omits the instability caveat**: master §6's *"at least TWO named flaky tests"* bullet marks *"a single run is not evidence — repeat and ID-diff"* **binding**, and that bullet is **not** superseded by the gate block (which supersedes only the baselines and the "which database" block). **Measured: neither flaky test is a member of the 21**, so a flake presents as a **NEW** failing ID in D23's diff — read as a regression — the exact misreading C9's own subset bullet prevents one bullet earlier. **N1** Redis ships without its 23-failed/2-errors diagnostic (master §6's four-element standard IS met, hence a note). **N2 — decided the other way, NOT a finding against the implementer**: the closing "must snap down" is a near-verbatim lift of §5.4 and obligation 5; the tension with §6A C's "no visible snap is required" is **between authority sections**, and a client CAN obey both (baseline vs rendered value) — routed as a lesson to the intention. **N3 — P7 measured: the double dependency is on FOUR projections, not one**; not a graph-vs-code discrepancy (dependency real, diagnostics 0) so not filed on the discrepancy route — owner **Card 1**, recommendation promote as-is. **Nothing promoted, rejected or edited.** Verified at source: P2's three price-scenario clauses (the load-bearing one holding because `status` derives from `evaluation.allowed_worker_minutes`, not worked time); P5's two claims exactly; C4's **both** amendments (§§2.3A+3.4A, and the **eight**-row list checked row-by-row against §2.5A); `app/pytest.ini`'s `-n 6 --dist loadfile`; §4.1's cost passage term-for-term against §3.4A including the corrected `min(...)` denominator. **Evidence: 0 L4** (gate stamp cited by tree identity); one L1 docs guard **59 passed** at `31e6634`, clean — new evidence, this tree differs from the last stamp in `docs/`. **No mutation probe applied**; nothing mutated. Handoff: `handoffs/reviewer/2026-08-22_phase4_review_r3_handoff.md`. |
...
- **A criterion that compresses an authority's sentence quotes its operative words**
  (review r3, lesson 1 — earned in a *published* deliverable). Plan 4's C5 paraphrased
  intention §5.4's *"record deletion … is not named to the client"* into *"record deletion
  NOT named as a cause"*. The narrower rule was **met** while the authority was **not** —
  the document named the event as a non-cause — and every downstream actor was correct
  against the artifact in front of them: the implementer against C5, the coordinator
  against C5, and only the reviewer, who went to §5.4 itself, saw the gap. A paraphrase in
  a criterion is a second source of truth with no changelog. Quote the clause.

- **A phase that writes prose asserting a mechanism owes a per-claim source-verification
  criterion** (review r3, lesson 5). Plan 4's C6 required "five nodes updated"; five were
  updated. **Nothing in C1–C9 required the new descriptions to be _true_.** They were —
  all five verified at source by review r3, every clause holding — but that was a probe the
  review prompt had to invent, not a criterion the plan carried, and a probe nobody invents
  is a claim nobody checks. When a phase's deliverable describes code, one criterion
  requires each asserted mechanism verified at its own source, per artifact. This is the
  documentation-phase analogue of rule 3 (invariants proven on the production code path).

- **An "authoritative supersedes" banner states what it does _not_ supersede.** A block
  that opens "read this before citing anything below" is read as retiring everything below
  it. §6's 2026-08-22 gate block retired two baselines and the database question, while the
  instability, timezone and residue bullets beneath it stayed true — and a published
  baseline lost the instability caveat exactly that way (review r3, S2). Enumerate the
  survivors in the banner.
...
- **⛔ THE GATE IS SATISFIED — 2026-08-22. READ THIS BEFORE CITING ANY BASELINE BELOW.**
  ...
  - **What this block does NOT supersede — added 2026-08-22 (review r3, lesson 3).** The
    banner above says "READ THIS BEFORE CITING ANY BASELINE BELOW", and a reader who takes
    it literally drops facts that are still true. It supersedes **the two baselines below
    and the "which database" block, and nothing else.** These survive it and are carried
    forward with every citation: the **⚠ Suite instability** bullet (two named flaky tests
    plus a third whose identity is unrecoverable — "a single run is not evidence"), the
    **host-timezone** bullet, and the **residue** bullet. A published baseline that omits
    the instability caveat is incomplete under §7's schema — that is how review r3's S2
    happened.
...
- **⚠ Suite instability — at least TWO named flaky tests** (named after 21 measured runs,
  `simple_valuation_editor/master_plan.md` §6 carries the full evidence). **Full node IDs,
  measured at source 2026-08-22 (review r5, N5) — publish these, never the basenames:**
  ...
  This bullet carried them as basenames for two months and a published baseline inherited
  that form into a block whose own instruction is *"repeat and **ID-diff**"* — a mechanical
  operation over full node IDs, which a basename does not satisfy without a lookup.
  **A document that instructs a diff makes every identifier in it mechanically comparable.**
...
**Schema amended 2026-08-21 (re-review r3, P5) and again 2026-08-22 (review r3, S2).
A baseline is published as `failure-ID set + tree identity + database identity +
reachable services + KNOWN INSTABILITY`, with the count explicitly subordinate.**

**The 2026-08-22 addition — known instability — and why it is a schema field rather than
a §6 bullet.** A published set is a *comparator*: the consumer diffs against it. The
suite's named flaky tests are **not members** of the published set, so a flake cannot
present to the consumer as a missing ID — it presents as a **new** failing ID, which
reads as a regression they introduced. That is exactly the misreading the subset bullet
prevents for the 26→21 transition, reproduced one bullet later by omission. Every
published baseline therefore carries: the named intermittent tests **as full node IDs**;
the third whose identity is unrecoverable, **stated with its direction — unlike the two
named ones it was observed as a MEMBER of the published set, so it can only ever remove an
ID, never add one, which is why the set can shrink as well as grow**; and the binding
consequence **"a single run is not evidence — repeat and ID-diff."**

**Amended again 2026-08-22 (review r5, N4/N5).** The direction clause was a parenthetical
gloss here — *"(so the set can shrink too)"* — and the first baseline published under this
schema carried the field and dropped the reason. **A schema field's parenthetical is where
the field's purpose hides:** when a field exists to prevent one specific misreading, the
field text states the misreading rather than glossing it. Same round, same shape: the named
tests must be full node IDs, because the block's own instruction is a mechanical diff. Earned twice: once here, once
independently in the inline-valuation project. It lived in a §6 bullet that a publisher
had no reason to read; it now lives where publishing happens. Phases 1 and 2 were published as a count against a bare SHA, which omits
...
| **4 — PIPELINE CLOSE** | **2026-08-22** | **`dc76db8`'s `app/` tree** — phase 4 changed nothing under `app/`, so the post-approval application tree **is** the isolation project's gate tree; `git diff dc76db8 HEAD -- app/` empty, re-verified at review r5 | **21 failed / 2576 passed**, collection **2597**, 50.61 s — **the new runner**: six xdist workers, `--dist loadfile`, **Redis reachable** (without it: 23 failed / 2 errors), each process on its own database cloned from `beyo_test_main_template` | **the 21, enumerated in full in the frontend handoff** — a strict subset of the 26 above, five removed / zero added, **derived twice independently** (coordinator against the isolation project's enumeration at fold time; review r5 against §6's 26 — both directions empty). **Known instability: the two named intermittent tests are NOT members of the 21; a third, unrecoverable, IS — so the set can shrink as well as grow. A single run is not evidence.** |

**This is the baseline `narrow_typical_work_times` D23 consumes** — published in
`docs/handoff/to_frontend/HANDOFF_TO_FRONTEND_live_working_time_clock_20260822.md` §7,
which is the artifact a successor should read, not this table.
