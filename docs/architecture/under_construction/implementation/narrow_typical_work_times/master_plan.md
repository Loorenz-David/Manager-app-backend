...
| 2 | Statement extension: spec→predicate, K-spec shape, HC-4 + §12 measurements | `IMPLEMENTED` | 2026-08-23 | Codex | Round 1 implemented after task-0 transcription; code accepted and §12's conditional acceptance met (11 rows, copies disclosed). **Fix round 2 closed the coordinator findings:** C5 now asserts literal section count/median across K and K=0 and catches `SUM*2`; C2(d), C3, C7 and C10 fixtures are fail-capable; all 7 missing mutations were run with IDs; C8's outer-attachment count-vs-median disclosure and the harness label are recorded. Focused perimeter is green; known three clock-in-code IDs remain out of perimeter. Checkpoint and handoff follow. |
