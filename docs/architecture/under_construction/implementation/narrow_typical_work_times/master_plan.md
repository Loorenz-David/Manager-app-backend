...
| 4 | Division contract + production-time + budget-allocations | **`CHANGES_REQUESTED`** | 2026-08-24 | Opus 5 (re-review r2) | Implemented and verified against the phase perimeter; focused/domain/service/docs evidence and named mutation probes are recorded in the implementer handoff. Full approval gate remains for reviewer. **CONSUMED 2026-08-23 → `CHANGES_REQUESTED`: 4 blocking / 2 should-fix / 0 cards.** Production engineering sound; the round is owed for **evidence**. **B1 is the phase's defining defect and it is measured:** C9(a)'s "immutable pre-refactor baseline" is **written by the test itself when absent** — snapshot removed → **8 passed**, file re-created from post-refactor output, and the regenerated copy carries `typical_resolution`, a key that does not exist pre-refactor. The handoff also admits the committed numbers were *"reconciled manually… rather than regenerated"*. A hand-edited baseline behind a self-healing read — the exact `f(x)==f(x)` vacuity §4 forbade in writing, at the criterion the projection spent blocking L4 to create. **B2:** C8, C10 and C11 have **no committed test** (+10 tests total for the largest phase); C10 deferred by the handoff to "reviewer follow-up". **B3:** ledger has 16 rows against **21** named mutations; C10(i)/(ii) *"verified by source inspection"* — §9: a never-run mutation is not evidence. **B4:** task 0's red baseline recorded nowhere. **S1:** baseline compared by count, not by the 21-ID set. **VERIFIED GOOD:** perimeter exact 14/14; **task 9c done** though it could not fail the suite; **C12's review half passes, diffed programmatically — +8/+6 keys and exactly 4 value changes, all `allocation_method` v1→v2; no number moved**; C2 absence half ∅; the C19 tripwire survived (17 passed, `Fraction` 0× in both services); graph evidence **zero spans**. **FIX ROUND 2 (in flight) FOUND THE PHASE'S FIRST PRODUCTION DEFECT — via C10(d).** In a **mixed-spec batch**, `get_task_budget_allocations` keys rows `(section_id, 0..K-1)` (`:150-151`) while a category-less task looks up `(section_id, None)` (`:254`) — a key that never exists — so it publishes `insufficient_sample`, contradicting task 8 / §3B B1 which require `narrowed_* := section_*` with a section-wide basis. Fix is safe because the section-wide columns are **spec-independent** (`section_count`/`section_percentile` filtered by `qualifying` only). **C9 did not catch it: C9 is the `K == 0` path, C10 the `K >= 1` mixed path.** ⚠ **The coordinator first adjudicated this as a fixture defect and was WRONG**; the implementer refuted it by measurement (27 -> 32 section totals, identical failure), found the real cause and reverted cleanly. Recoverable only because the ruling shipped with its own refutation condition -> new §9 rule. Production fence lifted for this one fix via `correction2` prompt (round-2 prompt never edited — reported against twice). **ROUND 2 + CORRECTION 2 CONSUMED 2026-08-24 -> `REVIEWING`.** All 4 blocking + 2 should-fix closed. The production fix is correct **and safe in the way that mattered**: the new `spec_index is None` branch hard-sets the narrowed slots to `None, 0`, so no spec-0 population can be attributed to a category-less task (Critical rank 2). `K == 0` path untouched, no fabricated key inserted. **L4 2687/21/1 with a programmatic 21-ID diff of ∅/∅**, and +3 over round 1 is exactly C8+C10+C11 — ledger and suite describe the same work. Ledger 22 rows = the 23 named (corrected at review — see S4) + the new anti-regression; the 5 previously-unrun mutations all executed. **B4 closed as honestly absent** rather than reconstructed. Notes: **N1** handoff mis-states a literal (`uniform_basis_v2` — a string nowhere in the repo; the code is right); **N2** C10's four rows share one test id so the bites are inferred not shown -> review; **N3** my own probe refuted — leaking spec-0 narrowed values is **inert**, reconcile guards twice (`:182`, `:277`). **REVIEW ROUND 1 (Opus 5) 2026-08-24 -> `CHANGES_REQUESTED`: 2 blocking / 5 should-fix / 11 notes / 0 cards** (`handoffs/reviewer/20260824_plan4_review_handoff.md`, tree `748e709`). **The production engineering was attacked and holds**: the `spec_index is None` fix is correct on all three attacks (`row is None` guard covers the soft-deleted shape; `(section_id, 0)` exists for every live section by construction; the `continue` is inside `if specs and ...` so `K == 0` is byte-for-byte unchanged), `apply_business_fallback` is term-for-term the deleted ladder, and **the refactor did not move a number** - leaf-set diff of both goldens shows 0 leaves removed, **0 pre-existing numeric leaves changed**, exactly 4 value changes, all `allocation_method` v1->v2. **L4 spent on variation, not repetition:** `-n 0` gives 21 failed / 2686 passed / 2 skipped and the failing set is **∅/∅ against the published 21-ID block** - the set is composition-stable in serial on a tree with three more integration files than phase 2's; the count delta reconciles via the `-n 0`-only skip at `test_database_isolation.py:118`. The implementer's stamp was NOT re-run. **B1 blocking: C5's rows (a)/(b)/(c) are not implemented as specified and row (b) is invisible at C5's own L2 scope** - republishing a zero section-wide median at count >= floor as `insufficient_sample` measures **346 passed**, and no `SectionTypicalEvidence` in `tests/` carries a zero section value at count >= floor; §4C exists solely to name that reachable shape. **B2 blocking: C1(c) and C13(c) never shipped as committed tests** though both criterion texts require it and §10 budgets C13(c) as an L1 test - substance verified TRUE, so §9's *the defect was the form* applies exactly; round 1's B2 was this class and these two were never in its list. **S1** the `_step_result` tolerance branch is unreachable production code publishing the impossible triple `(value, "section_wide", 0)`, asserted by nothing (**494 passed** under mutation), an undeclared divergence from §5 task 4. **S2** `test_item_economics_domain_walk_is_recursive` is `f(x) == f(x)`: `rglob`->`glob` alone leaves **4 passed** - escape 3's shape one level up, inside the file written to close it. **S3** C2's production-time half has no exact-literal assertion; both v2 literals read `e2_row`, a **budget-allocations** row - round 1's S2 mis-read the variable and the fix doubled the covered half. **S4 the ledger is two rows short: the plan names 23 named mutations, not 21** (the 21 was B3's arithmetic on a round where C8/C11 had no test) - **the reviewer ran both missing mutations and both bite**, C8 at `:198` and C11 at `:290` with a collateral C10 bite `[27] == [7]`, so no re-run is owed, only the count in three artifacts. **S5** C1(a)/(b) assert `f(a) == f(b)` with no literals and no non-emptiness guard, on the Critical rank 5 row. **16 reality checks recorded so the fix round does not re-verify them** (perimeter exact 14/14; C9(a)'s snapshot pre-refactor-shaped, read-only, comparing 24 numeric leaves, its `e7b2c41` re-capture moving only `uuid4` tokens; C11's exact literals both sides; C8's self-enforcing fixture precondition; C10's discriminating 7/9/11 counts; C1's three preconditions holding and C1(i) biting when correctly sited; §3B B3 guarded by 5 tests; task 9c accurate field-for-field). **A refutation against the reviewer itself:** the first C1(i) probe went green because it was sited on the settled `tsp_failed` step - C1's own precondition 2, and §9's *a probe that lands in the wrong place measures nothing*. 7 probes, 5 files, all md5-identical after revert, `app/` clean, 218 passed. **N9 routed to the owner** (the graph delta records contract tests, not §7's two projection nodes); **N11 routed to plan 5** (uniform fixture multisets); **N8 must be fixed before B2's test can be written** (C13(c) expects every non-`budget_division` hit to be a test fixture, but `get_task_price_scenario.py:14/134` imports the shared predicate). **FIX ROUND 3 CONSUMED 2026-08-24 -> `REVIEWING` (delta re-review).** Perimeter exact 8/8; **task 0's 33-row completeness table over every criterion row letter is why both blocking findings closed**. L4 2692/21/1, 21-ID diff ∅/∅, and +5 over round 2 is exactly the five new test functions. **B1 closed and BITING, measured with the reviewer's own mutation: 346 passed (invisible) -> 1 failed / 350 passed** on the new `test_c5b_…`. B2's C13(c) half is a genuine rglob walk with `assert files` + `assert hits` + named exceptions + a count pin. S1 branch gone and 23 literals converted; S2's guard now builds a nested module under tmp_path; S3 verified at `:206` on `e3`; S5 exact 3200/1600 both clocks both surfaces; N3 reads the constants. **S4: the handoff claims C8/C11 are transcribed and the 26-row table contains neither** — and **my own 'no re-run is owed' expired** when round 3 moved those tests from `:198`/`:290` to `:277`/`:351`; I re-ran both on THIS tree (C8 1 failed/15 passed; C11 the basis-string discrimination plus the collateral `[27]==[7]`), both reverted, md5 identical. **One finding remains and I authored it: C1(c)'s second root never existed** — L9 named 'this phase's evidence-construction helper' when evidence is built inline in two services that must carry the live-clock terms; round 3 bound it to a test-local helper. §6 C1(c) amended, second root struck, routed to the delta re-review. **DELTA RE-REVIEW r2 (Opus 5) 2026-08-24 -> `CHANGES_REQUESTED`: 0 blocking / 3 should-fix / 7 notes / 0 cards** (`handoffs/reviewer/20260824_plan4_rereview_handoff.md`, tree `24af53a`). Perimeter **exact 8/8** and **no coordinator probe residue** (`typical_filters.py` / `get_task_production_time.py` show no diff from `748e709`; my exit md5s match the coordinator's own revert values). **Both blocking findings close; B1 closed and BITING; no production change is requested** - everything owed is test/fixture code. **S1: C13(c) admits a FAITHFUL private copy in the one file it allowlists** - measured, import removed from `get_task_price_scenario.py` + local `def`, occurrence count preserved at 2 -> L2 **351 passed**, `test_c13c` green; the *disagreeing* copy is caught by behaviour instead (**3 failed / 348 passed**), exactly as C13's own note predicts. `hits <= allowed` + `count == 2` all hold over a copy because a `def` plus its call is also 2. Undeclared divergence too (2 of 5 terms, production root not repo root, rule 14) - substantively right (**40** production files carry SKIPPED/CANCELLED/FAILED) but must be declared; the different-name hole is closable, **0** files carry a set literal naming 2+ states. **S2: the `SelectedTypical` conversion re-created the impossible triple round-1 S1 deleted from production, in the fixtures** - `{"missing": None}` -> `(None,'insufficient_sample',0)` but `selected("missing", None)` -> `(None,'section_wide',0)`; invisible because `typical_basis` is unasserted in that file. **S3: C1(c)'s test still carries the root §6 struck** (`inspect.getsource(selected)`, `assert roots` tautology, a name promising a guard that does not exist); its surviving half bites (**1 failed / 222 passed** at `:514`). **Step 3 adjudicated: the row should never have had a second root** - striking it is correct; the one span-scoped structural claim that IS available is recommended as a plan-5 note, not a plan-4 obligation, because C1(i) discriminates the same hazard and was re-measured biting. **11 new reality checks**, incl. C5(b)'s zero proven a real median two ways (shared `filter (qualifying)` on count and percentile; and history 5->4 gives `(None,'insufficient_sample',4) == (0,'section_wide',5)`, one test red - the boundary is exactly the floor); **C6's mutation and C1(i) re-measured on this tree because round 3 rewrote the tests their retained ledger rows cited** (the S4 expiry class - both hold); **TZ at UTC/+14/-11 -> 351 passed each**, §10's two-TZ rule discharged for the new datetime fixtures. **Refutation against myself:** my first C13(c) probe was a *disagreeing* copy and three behavioural tests caught it - a mutant that is not the shape the criterion names measures a different criterion. **L4 runs 0, deliberately:** `git diff 3f8677c HEAD -- app/` empty, so round 3's 2692/21/1 stamp describes this tree and is cited; **the mandated approval-gate L4 with the 21-ID diff is owed on the tree that closes fix round 4.** 8 probes / 6 files, all md5-identical after revert, `app/` clean at exit, L2 green at 351. Notes: N1 `test_c2c` pools both roots so the goldens root's non-emptiness is unasserted (5th instance of the phase's shape); N2 the `glob` mutation reddens at the helper's `assert modules` and still never reaches the row's own sub-check (rule 12, 3rd generation); N3 C5(c)'s `>= 1` is a rule-2 disjunction (the plan's weakness); N4 rows 25/26 name a mutation without its site; N5 the handoff's C8/C11 sentence is false against its own table but the record is adequate via the coordinator's measurement; N6 a stray `3---` corrupts the round-2 prompt's frontmatter in the owner's tree (outside `app/`, untouched). **DELTA RE-REVIEW (Opus 5) 2026-08-24 -> `CHANGES_REQUESTED`: 0 blocking / 3 should-fix / 7 notes / 0 cards.** Both round-1 blocking findings **closed, B1 closed AND biting**; production engineering settled and not re-derived; **no production change requested** — fix round 4 is three test-file edits. **L4 runs: 0** — round-3's stamp consumed by citation on a byte-matched tree, round-1's serial comparator deliberately not repeated, budget spent on **variation**: 8 probes / 6 files, a **3-point TZ matrix** discharging a §10 obligation never run on these fixtures, a fixture-population variation proving C5(b)'s zero is a real median (5 green / 4 red at the floor), and **two retained ledger rows re-measured because round 3 rewrote their tests**. **S1: C13(c) is blind to a FAITHFUL private copy** — `hits <= allowed` + `count == 2` cannot tell an import from a local `def` + call site (verified structurally by the coordinator; reviewer measured 351 passed, green) — plus an undeclared rule-14 narrowing that was substantively right. **S2: the SelectedTypical conversion re-created the impossible triple in fixtures** — `selected()` defaults `basis="section_wide", count=0` and that file has **0** `typical_basis` assertions, so 23 rows publish an unreachable state invisibly. **S3: C1(c)'s test still carries the struck root.** **Two plan defects amended, both the coordinator's:** C5(c) shipped as `>= 1` where charter rule 2 wants `== 2`, and C13(c) never labelled which of its five terms were structural vs prose (three name enum members across ~38 production files). N6 stray `3---` in a published prompt **repaired and declared**. 4 rules -> §9. Approval-gate L4 owed **run, not cited**, on the tree closing round 4. |
