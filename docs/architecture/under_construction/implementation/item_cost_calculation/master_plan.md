...
- **Domain (pure, no I/O):** `app/beyo_manager/domain/item_economics/`
  ...
  **Registry decision (review N2, 2026-08-12):** the two structural Protocols
  `EvaluationSnapshot` and `TermSnapshot` join the public surface (phases 7/8 type
  against them), as do the marker constants `REDERIVE_SKIPPED` and
  `REDERIVE_MISMATCH` (R9-1). The module declares **`__all__`** with exactly this
  surface — stray re-exports (`Decimal`, `ROUND_HALF_EVEN`, `Sequence`,
  `ValidationError`) are not public API.
