...
published baseline therefore carries: the named intermittent tests, the fact that a third
one's identity is unrecoverable (so the set can shrink too), and the binding consequence
**"a single run is not evidence — repeat and ID-diff."** Earned twice: once here, once
