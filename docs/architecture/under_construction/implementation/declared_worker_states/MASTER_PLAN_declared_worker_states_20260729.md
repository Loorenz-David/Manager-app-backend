...
| 2 | `PLAN_declared_worker_states_phase2_derivation_20260729.md` | State machine + live reconcile + clock-out reconstruction read the new table (still inert — table is empty) | `needs_changes` (round 2) — F1/F2/F3/F5 fixed & verified at `aa0260a`; F3's fix regressed legacy manual-pause stickiness (G1 blocking: `/pause` clobbered by step pause, strands `/resume`), G2 (pin step→step stickiness), G3 (scope doc for `linear_timeline.py`). Fix cycle 2 via `codex_prompts/PROMPT_phase2_fixes_round2.md`. |
