...
| 2 | Statement extension: spec→predicate, K-spec shape, HC-4 + §12 measurements | `NOT_STARTED` | 2026-08-22 | coordinator | Projection **mandatory** (4 of the 5 Critical rows) — prompt dispatched (`prompts/reviewer/20260822_plan2_projection_prompt.md`, Opus 5). Acceptance **conditional** on the 10-row measurement doc. **First tests-first phase** (§3). Carries C0 inherited from plan 1's re-review (N7, the parser's remaining families). |
