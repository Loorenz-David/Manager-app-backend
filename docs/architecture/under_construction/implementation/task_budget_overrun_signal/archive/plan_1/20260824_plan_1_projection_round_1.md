...
2. `docs/architecture/under_construction/implementation/task_budget_overrun_signal/plans/plan_1.md`
   exists and its Review log contains only the coordinator's 2026-08-24 round-0 fold entry.
...
and no-work-ahead boundary; exact `int`/`Decimal` inputs and the mandatory money-function call;
four-state precedence with both populated pairs; the wire-only currency sentinel; and the
closed public API surface. Configuration/plumbing receives only the depth needed to establish it
has no hidden decision.
...
`role: projection`, `round: 1`, `date`, `verdict` (`PROJECTED_CLEAN` or
