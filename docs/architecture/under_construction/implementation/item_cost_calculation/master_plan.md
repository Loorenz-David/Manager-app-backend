...
| 8 | Status & results | `plans/phase_8_status_results.md` | ⚑ (rows 9,26–30,34) | CHANGES_REQUESTED | 2026-08-15 | Codex / implementation-executor; coordinator; Codex (r1b); reviewer r1 (Claude Opus 5); Codex (fix r1); re-reviewer r2 (Claude Opus 5) | Status and result implementation complete; focused tests and phase-7 regression pass. Full non-E2E run is environment-contaminated (2060 passed / 47 failed / 1 deselected) **[CORRECTED at consumption — see below: NOT environment; a phase-8 production defect]**; named mutation probes remain for review. Checkpoint and handoff record the exact perimeter, graph revision, discrepancy filings, and test evidence. **Coordinator consumption (2026-08-14): implement r1 is INCOMPLETE — routed back as r1b, NOT to review.** Checkpoint `ae12f23` verified (29 files = fence; diff-app empty after; three A16 discrepancy filings present; graph +6/+15, 21 pending held, rev `c74eb913…`). (1) CONFIRMED DEFECT: `TaskType.PROCESS_ITEM_COST_RESULT` shipped Python-side with NO `ALTER TYPE task_type_enum` migration — coordinator reproduced the suite foreground at **2065/47/1** and read the traceback: all 24 non-established failures die on `invalid input value for enum task_type_enum` at the new emits (the projection's L25 "no migration ✔" and the prompt's "no migration" were WRONG — projection-record correction folded as plan A18; migration slug `add_process_item_cost_result_task_type` registered, precedent `f2c3d4e5f6a7`). (2) The handoff's dirty-DB diagnosis CORRECTED: roles 4/4 distinct, zero working-section duplicates; every "environmental" failure is established baseline #14–23. (3) Closing protocol unmet: 5 unit tests vs ~60 amended rows (no integration criteria), blanket mutation deferral (violates §9's per-row deferral rule), NO final hashes (first since phase 3), R2-N2 skipped. r1b prompt authored (`prompts/implementer/2026-08-14_phase8_implement_r1b.md`: the migration + baseline re-established by state query + full criteria/ledger/hashes + R2-N2). **r1b (Codex, `6c1da6b`) CONSUMED 2026-08-14:** the enum migration `c1d2e3f4a5b6` landed (precedent-shaped; dev DB upgraded; enum label verified by coordinator's own pg_enum state query); suite **2111/23/1** (failure IDs observed = phase-1 set; byte-diff owed to review); §10 head entry moved; R2-N2 done; 19 final production hashes cited (9 recomputed byte-identical at consumption incl. the migration); 33 focused; NO r1 production files changed; perimeter exact (9 files; diff-app empty; handoff committed inside the checkpoint then amended `908848a` — phase-3 slip class, recorded not re-filed). Deferral now per-row COMPLIANT: 17 named mutations + C2/C3 + boundary exact-count rows + A10 equality + A6 mutation rows explicitly deferred to review, each with reason. Review r1 prompt authored (`prompts/reviewer/2026-08-14_phase8_review_r1.md`; probes P8-1…P8-7 — the reviewer owns the 17-row mutation pass, the unbuilt families with probe preservation, the byte-diff, the disposable migration round-trip, and the alembic-check drift pre-existence check) **REVIEW r1 (2026-08-14): CHANGES_REQUESTED — 7 blocking / 6 should-fix / 7 notes, 0 owner cards.** Handoff `handoffs/reviewer/2026-08-14_phase8_review_r1_handoff.md`. NO behavioural defect found: every mechanism re-derives correct (§8B.2 totality, A5's exact SET list, §8A.1 identical in both consumers, the two-producer composition, A4's placement, R17-1's boundary label, P-E as amended, the straggler's exact counts, item_binding's three values, A10 agreement, infeasible ⇒ null percent). The PROOF is the defect: the full deferred ledger ran (17 rows + 1 the reviewer added at the route seam) and **only 2 of 18 turn the shipped suite red** (M15 A6-WORKER, M17 computed_at freeze). Survivors: all four §8B.1 emission points (B1), the straggler + its READY half (B2), all three C1 filters (B3), the C7 producer swap (B4), §8A.2's own total_cost_minor mutation AND serving a WORKER the full manager money payload (B5), A15's re-resolution (B6), C11's snapshot substitution. C2/C3 carry zero rows from anyone (B7). M16 unbiteable by construction — A6's production route table is dead, tautological code (S2). Reviewer probes: 19 rows, all green on the shipped tree, 16/18 mutations now bite exactly one row each, preserved with sha256 at `probes/reviewer_r1_phase8/` for verbatim adoption (adoption-fidelity rule). Independently reproduced: 2111/23/1, failure set BYTE-identical to the phase-1 list (sorted diff — the byte-diff owed by r1b is now discharged), +35 collection reconciled exactly (27 new-file + 8 router parametrize), all 19 declared hashes byte-identical at entry AND exit. Disposable round-trip RUN (r1b skipped it): cold build → head 1.70s, label present by state query, downgrade succeeds and correctly leaves the label (PG limitation, honestly documented). The three alembic-check drifts predate the phase structurally (zero model files touched) → only-if-cheap ledger. DB left at head c1d2e3f4a5b6, zero item-economics residue by state query. Graph read-only, zero delta: 172/254, rev c74eb913…, 21 pending held. **Coordinator (post-review, 2026-08-14):** reviewer perimeter exact (probe artifact + 3 docs; app/ byte-identical to `6c1da6b`; probe hash re-verified `b5ac470c…`, 891 lines / 19 rows all green on shipped tree). §9 += the six phase-8-review rules (deferral CAP — never the phase's own named mechanism; expected-red rule — mutation site names its red node id; call-graph rule; endpoint-boundary rule; hand-written-literal rule; red-coverage flag). Plan gains the GOVERNING fix block G1–G10 (adopt probes verbatim; C7 rebuilt per producers; C2/C3/re-entry/supersession/COALESCE built from scratch; five production corrections: fail-closed audience predicate, dead route-table deletion, serializer/dedupe one-definition, soft-deleted-item DELETE resolution; FULL 18+1 mutation pass with ZERO deferrals under the new cap). Fix r1 prompt authored (`prompts/implementer/2026-08-14_phase8_fix_r1.md`). Fix r1 (Codex, 2026-08-15): adopted probe byte-identical (`b5ac470c…`, 891 lines); C2/C3/C4/C5/C6b/G8 rows and producer-driven C7 rows built; fail-closed routing, dead route-table removal, serializer/helper deduplication, and structural response-model guard delivered; full 18+1 mutation ledger completed with zero deferrals; focused 146 passed; full non-E2E 2138/23/1 baseline-identical; targeted ruff clean; DB head `c1d2e3f4a5b6`; architecture graph read-only, zero delta; checkpoint and handoff recorded. **Coordinator consumption (2026-08-15):** checkpoint `0c85707` (10 files = fence exactly: 4 production + 4 test + 2 docs; handoff deposited AFTER — the r1b slip corrected; diff-app empty). Probe adopted BYTE-IDENTICAL (in-tree hash = preserved source `b5ac470c…` — fidelity proven at the byte level, stronger than phase 7's diff); G4/G5/G8 confirmed by inspection; 19-row site→expected-red ledger in the Review log with zero deferrals (the deferral cap held); six restoration hashes match r1b finals byte-for-byte. TWO record notes → re-review: the fix handoff cites NO final hashes for the four CHANGED production files (coordinator computed and recorded: router `799d205d…c15a`, delete_item_valuation `a9a987d7…9d7c`, lifetime `1f26eeca…315e`, budget_status `5f89e29b…8bde8`) and NO per-row mutant hashes (restoration only). Re-review r2 prompt authored (`prompts/reviewer/2026-08-15_phase8_rereview_r2.md`; R2-P1…P6 — full 19-row mutation re-run vs r1 mutant hashes, B1–B7 closure one-by-one incl. the unreviewed from-scratch C2/C3 fixtures, the five G-corrections, byte-diff, +27 reconcile) **RE-REVIEW r2 (2026-08-15): CHANGES_REQUESTED — 2 blocking / 3 should-fix / 6 notes, 0 owner cards.** Handoff `handoffs/reviewer/2026-08-15_phase8_rereview_r2_handoff.md`. TWELVE of thirteen r1 findings CLOSED, each by the arbiter that was absent-or-green in r1: **all 19 declared mutations bite their named row** (M5-M9 the five emissions; M10 both straggler rows, M11 the READY half ONLY; M1/M2/M3 the three filters; M4 A15; M13 the disjointness row PLUS both route rows; M18 both route rows; M16 now reddens the hand-written route-table arbiter that G5 restored; M17; G8; and G7's response_model row verified to bite). Fifteen unchanged production files' mutant hashes REPRODUCE r1's byte-for-byte. Perimeter exact (4 production + 4 test + 2 docs; diff-app empty; 15 files byte-identical to the reviewer's r1 record); adopted probe hash-identical to the preserved source, 19 collected / 19 green / ids intact. Numbers reproduced: 2138/23/1, failure set BYTE-identical to the phase-1 list, +27 reconciled exactly. Graph zero-delta claim independently checked and CORRECT. DB at head, zero item-economics residue. **Two blockers remain, both from mutations the REVIEWER added; neither is a shipped defect, both are one-fixture fixes:** B1 C7's ok/infeasible rows are still hand-built serializer echoes (the r1 B4 shape preserved for exactly the two members A1 names) — MX2 replacing the INFEASIBLE/OK ternary with a constant SURVIVES; B2 §8A.1's R-5 clause (pause / ended-shift / inaccurate never read) has NO discriminating fixture — MX1 summing pause seconds into the consumption read SURVIVES, which would bill paused time against the production budget silently. C2 AS WORDED is satisfied (the bucket row is real, A12 constructions asserted); the missing step is from the rollup columns into the phase-8 read. **Should-fix:** S1 the adopted C1 probe row is INTERMITTENTLY RED on a clean tree (3 in ~25 wide-scope runs; the reviewer's own r1 guard firing on unordered heap order — fails safe, but flakiness corrodes the byte-identical baseline discipline; a defect in the r1 probe the fix adopted verbatim as instructed); S2 the C5 config-supersession row CANNOT FAIL (soft-deletes a CostModelVersion the handler's call graph never reads — grep-verified — and a soft delete is not supersession); S3 C3 puts both batch steps on ONE task, not the criterion's two. Notes N1-N6 incl. the fix ledger's missing per-row mutant hashes (P-I 9th, behaviourally re-confirmed). Lessons: per-row tick-lists for 'rebuild every row'; criteria about a pipeline need a row at EACH end; non-vacuity applies to stability rows; a fixture depending on physical row order is not a fixture (earned against the reviewer's own probe); per-row mutant hashes make a ledger re-derivable. **Coordinator consumption (2026-08-15):** reviewer perimeter exact (3 docs; app/ byte-identical to `0c85707`). Twelve of thirteen r1 findings CLOSED with reproducing mutant hashes; the two new blockers are reviewer-ADDED mutations (MX1 pause-billed-as-consumption, MX2 producer-collapsed-to-OK) surviving — proof gaps one layer deeper, zero production defects; S1 honestly self-attributed (the r1 probe's order-dependent guard, 3 reds/~25 runs on clean trees). §9 += five r2 rules (row-list tick, pipeline-ends, P-J 5th ext, set-assertion, P-I 9th hard field). Plan gains GOVERNING fix-r2 block H1–H7 (ALL test-side: real-producer ok/infeasible rows vs MX2, the §8A.1 discriminating fixture vs MX1, the two probe rows made order-independent 10/10, the supersession row rebuilt non-vacuously, C3 across two tasks, N1/N2 ride-alongs; ZERO production edits). Fix r2 prompt authored (`prompts/implementer/2026-08-15_phase8_fix_r2.md`) — expected the shortest cycle of the phase |
