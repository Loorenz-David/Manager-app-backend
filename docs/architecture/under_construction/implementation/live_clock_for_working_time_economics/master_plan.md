...
|---|---|---|---|---|---|
| 4 | Closeout handoff + the five-node graph delta | **IMPLEMENTED** (fix r4) | 2026-08-22 | Codex | **Fix r4 closed S1, S2, N1 and N2 in the declared docs perimeter; no graph or `app/` changes.** §5 mode 2 no longer names record deletion; the closing rule names the smoothing baseline; §7 publishes the two named intermittent tests, the unrecoverable third intermittent test, the repeat-and-ID-diff rule, and Redis's 23-failed / 2-errors diagnostic. Docs guard: 59 passed. Handoff: `handoffs/implementer/2026-08-22_phase4_fix_r4_handoff.md`. Checkpoint follows.**
