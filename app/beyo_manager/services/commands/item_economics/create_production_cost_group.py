...
from beyo_manager.services.commands.item_economics._common import audit, translate_integrity_error
from beyo_manager.services.commands.item_economics.requests import parse_production_cost_group_create_request
from beyo_manager.services.commands.utils.transaction import maybe_begin
from beyo_manager.services.context import ServiceContext


async def create_production_cost_group(ctx: ServiceContext) -> dict:
    request = parse_production_cost_group_create_request(ctx.incoming_data)
    async with maybe_begin(ctx.session):
        ...
        await audit(ctx, "production_cost_group.created", "production_cost_group", group.client_id)
