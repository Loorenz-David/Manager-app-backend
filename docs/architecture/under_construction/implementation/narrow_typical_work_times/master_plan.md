...
| 2 | Statement extension: spec→predicate, K-spec shape, HC-4 + §12 measurements | `PROMPT_READY` | 2026-08-22 | coordinator | Projection: AMENDMENTS_REQUIRED, 33 rows / 9 blocking / **1 owner card answered (D26)**. 9 of 14 criteria were not transcribable — folded into plan 2 §6A. Acceptance is now **measurement, not a threshold** (§12A); shapes corrected to **20-task pages** + one 50×20 ceiling row. Implementer prompt dispatched (`prompts/implementer/20260822_plan2_implementer_prompt.md`) — **task 0 is transcribe-the-criteria-first**. |
