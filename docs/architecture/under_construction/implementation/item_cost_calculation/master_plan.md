...
| 6 | Legacy money migration & API bridge | `plans/phase_6_legacy_migration_api_bridge.md` | ⚑ (rows 31,32) | **CHANGES_REQUESTED** | 2026-08-14 | Codex; reviewer r1 (Claude Opus 5) | Projection r0 amendments consumed; journaled migrate-and-drop plus reject-iff-non-null bridge implemented. Four router request bodies retain the legacy fields, four command request schemas reject only present/non-null values with the exact D1 ValidationError, five write paths and two serializers no longer carry the legacy surface, and the router README mirrors were removed. Data migration includes P1/P2/P3 preflight refusal, idempotent journal/copy, current-valuation collision protection, Python-side `ival` ids, exact legacy-to-valuation mapping, reversible downgrade, and retained enum ownership; drop migration removes only the three `items` columns. Focused 30 green (29 phase-6 rows plus the phase-5 synthetic tie row); full suite 1997 passed / 23 established failures / 1 deselected; ruff and diff checks clean; configured development DB at `be9dfe42a035` head with zero journal rows, zero legacy columns, and zero remaining disposable phase-6 databases. Tests, mutation ledger, review log, checkpoint, and handoff recorded below.. Implementer r1 (Codex, `b940309`, final; handoff after): data migration `5420acc6a7b3` (P1/P2/P3 report-first, journal, Python-side ival ids, exact mapping) + drop `be9dfe42a035` (create_type=False downgrade); D1 bridge on the 4 schemas; router bodies retained (zero diff); 9-surface serializer removal; 29 focused + tie-breaker; full 1997/23/1 (= 2020 selected, coordinator re-collected exact, +29); head MOVES to be9dfe42a035; graph +1 journal node (7 pending). Coordinator consumption -> review probes: P6-A undeclared item_upholstery_requirement.py change (expected R2-1 ownership flip — verify at the migration site + cold-build end state) + five write-path files beyond the prompt fence (coordinator gap, §10.2-required — verify diffs remove only the three-key writes); P6-B ledger lives in the Review log (verify completeness, re-run ≥3); P6-C focused split + items 479->480 attribution. Review r1 prompt authored (`prompts/reviewer/2026-08-14_phase6_review_r1.md`) **Review r1 (Claude Opus 5): CHANGES_REQUESTED — 2 blocking, 4 should-fix, 11 notes, 1 owner card.** Perimeter exact (21 files, tree clean before and after every probe); suite re-run foreground **1997/23/1** with the failure set byte-identical to the phase-1 baseline; ruff clean on the phase-6 perimeter (131 repo-wide errors predate it); DB at head `be9dfe42a035` (journal 0, legacy columns 0, zero disposables); graph read-only, 7 pending (1 new + 6 prior), zero adjudications. **B1 — §10A.1 post-condition 2 is a TAUTOLOGY:** both sides of the created-valuation check range over the same rows by construction, so a partial copy passes and `be9dfe42a035` then drops the money — EXECUTED, one eligible seeded item + `rows[1:]` mutation gives `alembic upgrade` exit 0 with 0 valuations; verified correction (`8f5bf7ce…`) keeps the shipped tests green and aborts the mutant (exit 1, version rolled back). **B2 — the eligibility predicate filters `is_deleted = false` only:** EXECUTED on a seeded disposable with the unmutated migration — an item whose only valuation is SOFT-DELETED is re-valued (the exact outcome §10A.1(c)'s heading forbids, matching the verbatim predicate printed below it — the clause contradicts itself, owner card 1), and an item whose only valuation is SUPERSEDED is skipped though it has no CURRENT valuation; neither row exists in C1's fixture set. **S1** C5's nine-row census is one assertion wearing nine labels (`endpoint_id` never consumed; re-exposing all three keys at `upholstery_orders_query.py:496` leaves all 27 phase-6 nodes green) — and D18 says these are the only arbiters this surface will ever have; fixture is a SimpleNamespace, not an ORM Item. **S2** the refusal rows never assert the row report (stripping every client_id leaves both migration tests green) and one message carries all three P-tokens, so the token assertion cannot discriminate. **S3** C4's 12 rows are schema-level, 1 of 4 surfaces proven end-to-end — reviewer probe confirms all three item routes DO return 422 + the exact envelope, so evidence gap, not defect. **S4** the enumerated tables are for-loops in two monolithic tests, zero parametrize ids (D23 unmet). **P6-A resolved:** the upholstery change is exactly the R2-1 one-line create_type flip; cold build empty→head exits 0; the five extra write-path files remove only the three-key writes. **P6-B resolved:** 2 of 5 ledger baseline hashes do not match the checkpointed files (tasks-requests, items-requests) and probe 1's red set is INCOMPLETE (re-run reddens 2 nodes, not 1); probes 2/3/5 reproduce, 3 and 5 byte-identically; reviewer added the shared-helper mutation (8 nodes red). **P6-C resolved:** 18+9+2 = 29 collected, the tie row adds 0 (assertion inside an existing test), 1991+29 = 2020; `items` 479→480→481 is the known suite-wide residue class. N2 D9's "2 → 1" enum arithmetic is wrong at head (the journal's snapshot column is a second user). Anchor-spans service delivered for all 7 pending items (all exact; `node:table-item`'s summary drifted :51→:46 — same §8 maintenance edit as its stale sentence). Lessons: a production post-condition owes a named aborting mutation (P-J ext to non-test guards); an unconsumed parametrize id makes N rows one row (P-V ext); an intention clause whose prose and verbatim predicate disagree cannot be discharged "verbatim"; a row-report criterion owes an assertion on the report's contents. **Review r1 (Claude Opus 5): CHANGES_REQUESTED — 2 blocking, 4 should-fix, 11 notes, 1 owner card.** B1 the in-migration pc2 guard is a TAUTOLOGY (executed: rows[1:] skip -> exit 0, 0 valuations, then the drop destroys the money; verified correction executed R9/R10). B2 the round-14 eligibility predicate contradicted its own heading — soft-deleted-only items were RE-VALUED (executed on a seeded disposable). Card 1 ANSWERED: leave deleted prices deleted -> R15-1 (predicate = NOT EXISTS any valuation row; never-valued items only). S1 census = one assertion wearing nine labels (R3 probe green); S2 row reports content-free; S3 3 of 4 bridge surfaces harness-proven; S4 for-loops. P6-A resolved legitimate (R2-1 flip exact; cold build clean; write-path diffs verified line-by-line); P6-B 3 of 5 ledger rows reproduce, 2 declared against pre-checkpoint baselines; P6-C reconciled (tie row adds 0 to collection; items residue class). Coordinator: R15-1 folded; §9 += P-J 4th ext (production guards owe abort mutations), P-V 3rd ext (unconsumed params), projection decidability rule, row-report rule, end-state counting rule; plan gains the GOVERNING fix-r1 block; fix-r1 prompt authored (`prompts/implementer/2026-08-14_phase6_fix_r1.md`) |
